...
#     default, no identifying user info is sent to the server.  Options include
...
#     Requires Perl 5.
#
#   Copyright (C) 1996, 1998-2017 by James Marshall, james@jmarshall.com
...
#
#     CGIProxy can run in any of four ways: as a CGI script, as a mod_perl
#       script, as a FastCGI script, or with its own embedded Web server.  For
#       the first three, you'll need a working *secure* Web server first, one
#       configured to support the way you want to run CGIProxy.  For the
#       embedded server, you'll need an SSL certificate / private key pair.
#       To get a free SSL certificate for your server, we recommend using the
#       Let's Encrypt service, at https://letsencrypt.org ; it's a certificate
#       authority (CA) that grants free SSL certificates, and the service is
#       run by people we trust (as of early 2017).
#
#     Starting in version 2.2.1, CGIProxy has an installation wizard that handles
#       most of what you need to install it.  After copying nph-proxy.cgi to your
#       server, run "./nph-proxy.cgi install" (or in Windows, "perl nph-proxy.cgi install").
#       This will:
#         - ask you some questions
#         - let you change any settings with a configuration menu
#         - create the directory structure under $PROXY_DIR (usually $HOME/cgiproxy/)
#         - create the configuration file $HOME/cgiproxy/cgiproxy.conf if it doesn't exist
#         - install all non-standard Perl (CPAN) modules required by CGIProxy
#         - create an empty database
#         - copy nph-proxy.cgi into place and modify it as needed
#         - add a cron job to purge the database every night
#
#     When installing the Perl modules, the installation wizard will use the
#       system's package manager if running as root.  Supported package
#       managers are dnf, yum, and apt-get.  If none are found, or if not
#       running as root, the cpan utility will be used.
#     When the Perl modules are installing, there will probably be a lot of
#       scrolling text.  You can ignore this, and hit <enter> if asked any
#       questions.
#     To *only* install the Perl modules, and not do any other installation
#       tasks, run "./nph-proxy.cgi install-modules".
#     If you run the installation wizard as root, the file permissions and
#       ownership of all directories and files will be set to be as secure
#       as possible.  However, even if you can't install as root, this will
#       be done as well as possible and the script should still work.
#
#     Also starting in version 2.2.1, you no longer need to edit this file to
#       configure it.  Instead, use the simple configuration menu by running
#       "./nph-proxy.cgi config".  Exception:  If you need to change $PROXY_DIR,
#       then the menu won't do it--  you need to either set it in this file, in
#       the configuration section below, or run the "install" command (which
#       asks you for $PROXY_DIR and modifies this script to set it below).
#
#     To see a simple usage message, run "./nph-proxy.cgi -?".
#
#
#   CONFIGURATION:
#     . After installing, use the configuration menu by running
#         "./nph-proxy.cgi config".  You should never need to edit the
#         configuration in this script file.
...
#         purging.  On Unix and Mac, the installation wizard creates the cron job
#         automatically.
...
#     . If you plan to run CGIProxy as a FastCGI script, see the configuration
#         section "FastCGI configuration".
#     . If you plan to run CGIProxy using its own embedded server, see the
#         configuration section "Embedded server configuration".  You'll need
#         a certificate and private key (key pair) in PEM format-- we recommend
#         using Let's Encrypt to get a free key pair, and their Certbot for a
#         tool to automatically manage it.
...
#       filename with "nph-".  However, it can usually run as a non-NPH script
#       by using the $NOT_RUNNING_AS_NPH option.
#
#
#   TO USE:
...
#     Remember that most of the delay experienced by the user is from waiting
#       on two network connections, which we have little control over.  The
#       advice below only applies if your server CPU is getting overloaded.
#     Visiting sites with a lot of JavaScript and Flash are most likely to
#       overload your CPU, because modifying those is what takes most of the
#       time.
...
#       or https://jmarshall.com/tools/cgiproxy/install.html .
#     If you can't use mod_perl, try using FastCGI.  You'll need to configure
#       your Web server to use FastCGI.  For more info, see the FastCGI
#       documentation, or https://jmarshall.com/tools/cgiproxy/install.html .
#     If you can't use mod_perl or FastCGI, try running CGIProxy as its own
#       embedded server.  You'll need a key pair (certificate and private key).
...
#       JavaScript or Flash holes.  Please tell me if you find any.
#
#
...
# Try to load any optional modules

# Math::Random::Secure provides a cryptographically secure version of rand() .
eval { require Math::Random::Secure ; Math::Random::Secure->import(qw(rand srand irand)) } ;


# Need BEGIN{} block here to set @CONFIG_VARS in time for next "use vars" statement.
use vars qw(@CONFIG_CATEGORIES  %CONFIG_CATEGORIES  @CONFIG_VARS) ;
BEGIN {

    # Do it this way to keep order of categories
    @CONFIG_CATEGORIES=('server/network environment',
			'FastCGI',
			'embedded server',
			'database',
			'common options',
	...
	([qw($RUN_AS_USER  $RUN_AS_GROUP
	     $SECRET_PATH  $LOCAL_LIB_DIR
	 ...
	 [qw($FCGI_SOCKET  $FCGI_MAX_REQUESTS_PER_PROCESS  $FCGI_NUM_PROCESSES)],

	 [qw($CERTIFICATE_FILE  $PRIVATE_KEY_FILE  $EMB_USERNAME  $EMB_PASSWORD)],

	 [qw($DB_DRIVER  $DB_SERVER  $DB_NAME  $DB_USER  $DB_PASS  $USE_DB_FOR_COOKIES)],
	 ...
	 [qw($DEFAULT_LANG
	     ...
	     @ALLOWED_COOKIE_SERVERS  @BANNED_COOKIE_SERVERS
	     ...
	     $PROXIFY_SCRIPTS  $PROXIFY_SWF
	     $ENCODE_URL_INPUT
	     $USER_IP_ADDRESS_TEST  $DESTINATION_SERVER_TEST
	     %REDIRECTS)],

	 [qw($INSERT_HTML  $INSERT_FILE  $ANONYMIZE_INSERTION  $FORM_AFTER_INSERTION
	 ...
	 [qw(@PROXY_GROUP
	     ...
	     $USE_PASSIVE_FTP_MODE  $SHOW_FTP_WELCOME
	     $PROXIFY_COMMENTS
	     $USE_POST_ON_START
     ...
     @CONFIG_VARS= map { @{$CONFIG_CATEGORIES{$_}} }  @CONFIG_CATEGORIES ;
}


# First block below is config variables that aren't in the config file, second
#   block is sort-of config variables, third block is persistent constants,
#   fourth block is would-be persistent constants (not set until needed),
#   fifth block is constants for JavaScript processing (mostly regular expressions),
#   and last block is variables.
...
use vars @CONFIG_VARS, qw(
   ...
   $RUN_METHOD
   $RUN_AS_USER_ID  $RUN_AS_GROUP_ID
   @MONTH  @WEEKDAY  %UN_MONTH
   %RTL_LANG
   $DB_FULLPATH  $DB_HOSTPORT  $DBH  $STH_UPD_COOKIE  $STH_INS_COOKIE  $STH_SEL_COOKIE
   $STH_SEL_ALL_COOKIES  $STH_DEL_COOKIE  $STH_DEL_ALL_COOKIES  $STH_UPD_SESSION
   $STH_INS_SESSION  $STH_SEL_IP  $STH_PURGE_SESSIONS  $STH_PURGE_COOKIES
   ...
   %MSG  @MSG_KEYS  $CUSTOM_INSERTION  %IN_CUSTOM_INSERTION  %OPTION_DESCRIPTIONS
   ...
   $status  $headers  $body  $charset  $meta_charset  $is_html  $is_xhtml
   %in_mini_start_form
   $does_write
...
#
# IMPORTANT NOTE:
# As of version 2.2.1, you no longer need to edit this file to configure
#   CGIProxy.  Instead, when you run "./nph-proxy.cgi install", a short wizard
#   will ask you for any required settings, and then will let you set any
#   of the configuration variables below with a simple command-line menu.  The
#   complete configuration will then be saved in the external file "cgiproxy.conf",
#   under $PROXY_DIR .
# Any time after that, you can run the configuration menu by running
#   "./nph-proxy.cgi config".
# That said, if you do edit the variables below, they will be used as the
#   initial configuration when "./nph-proxy.cgi install" is first run.  Once the
#   external configuration file has been created, this section will be ignored.
# There is one exception:  $PROXY_DIR .  The installation wizard asks you for
#   its value, which must be an absolute path.  Then, after the wizard copies
#   this script file into place, it modifies the line below that sets $PROXY_DIR
#   to reflect what you entered.  This special treatment for $PROXY_DIR is
#   because we can't put it in the configuration file, because we need it to
#   find the configuration file in the first place.
# The section below is still useful, as the comments above each config
#   variable are the most complete descriptions of them.  Look in here
#   if you need more info than the short descriptions displayed by the
#   configuration menu.
#
#**************************************************************************
...
# *************************************************************************
# *** This is the only configuration variable that cannot be set by using
# ***   the configuration menu.  Instead, the installation wizard asks
# ***   you for its value.
# *************************************************************************
# For certain purposes, CGIProxy needs to create files.  This is where
#   those will go.
# The installation wizard asks for the value of $PROXY_DIR , which must be
#   an absolute path.  Then, after the wizard copies this script file into
#   place, it modifies the line below to be whatever the proxy owner entered.
# If this is set to a relative path before running the installation wizard,
#   then it will be interpreted relative to the user's home directory.
...
#   the FastCGI server or the embedded server.  The installation wizard will
#   try to set permissions appropriately for this.
# Note that you need to use "\\" to represent a single backslash.
# Leading drive letters (e.g. for Windows) are allowed.
$PROXY_DIR= 'cgiproxy' ;


# Most initial configuration is in this routine.  (Ignore this if you don't know
#   what it means.)
sub config {


# *************************************************************************
# *** Note that these are now set in the configuration wizard, so you don't need to
# ***   set them here.  If you do set them *both* here, these values will be used
# ***   and the wizard won't ask you questions about them.
# *************************************************************************
# If you have root access and can run "./nph-proxy install" as root, then set $RUN_AS_USER
#   to either the username or numeric user ID that the script will run as, and
#   set $RUN_AS_GROUP as the group name or numeric group ID that the script will
#   run as.  When run as a CGI script or under mod_perl, these are usually the Web
#   server's username/group, or possibly the script owner's username/group if using
#   Apache with the suEXEC feature turned on.
# Setting these lets "./nph-proxy install" create the needed directories ($PROXY_DIR
#   and subdirectories) and a SQLite database file (if using SQLite) with the right
#   permissions and ownership.
# If you're using the embedded server, and you run this script as the root user
#   in order to use port 443, it's a good idea to change the user and group IDs
#   to something with fewer permissions.  You can do this too by setting
#   $RUN_AS_USER and $RUN_AS_GROUP.
# In any case, these have to be set to an existing user and group on the server,
#   i.e. CGIProxy doesn't create the user and group if they doesn't already exist.
# If these are not set, they will default to the owner and group of this script file.
# These are not needed when installing on a Windows server-- you don't need root
#   access to use port 443 on Windows.
#$RUN_AS_USER= 'nobody' ;
#$RUN_AS_GROUP= 'nobody' ;


# If CGIProxy can be accessed without a path in the URL (e.g. "https://example.com"),
#   then a censor could check if a site hosts a proxy by merely visiting that URL.
#   Thus, to help evade censorship, it's better to install this in an undisclosed
#   subdirectory.  $SECRET_PATH is that subdirectory.
# If this is left empty here (recommended), a random string will be generated
#   during initial configuration.
# If using CGI or mod_perl, then this subdirectory will be created when you
#   run "./nph-proxy.cgi install", CGIProxy will be installed in that directory,
#   and the path in the proxy URL will contain the value of $SECRET_PATH.
# If using FastCGI or the embedded server, the URL of your proxy will be
#   "https://example.com/secret" (replace "secret" with the value of $SECRET_PATH),
#   but an actual subdirectory will not be created or needed.
...
# If you don't have root access on your server, this tells CGIProxy where to
#   install (and later find) Perl modules using the local::lib module.  This
#   is almost always the "perl5" directory under your home directory.
# This can be either an absolute path (beginning with "/") or a relative path.
#   If it's a relative path, it will be relative to the script owner's home
#   directory (or on Windows, relative to the current user's home directory).
# If this is a relative path, it will be converted to an absolute path before
#   being saved to the configuration file.
# Be sure to follow any instructions about the environment variables after you
#   run "./nph-proxy.cgi install".
# On Windows, this will be stored as an absolute path in cgiproxy.conf ;
#   otherwise, when nph-proxy.cgi runs, it would not know the owning user.
$LOCAL_LIB_DIR= 'perl5' ;
...
#   and try running the script as a normal non-NPH script.
# NPH is supported on almost all servers, and it's usually very easy to install
#   a script as NPH (on Apache, for example, you just need to name the script
#   something starting with "nph-").
...
#   the port being optional (defaults to 1080).  You can also set it to just
#   a port number (with or without a leading ":"), and it will assume localhost.
# If your SOCKS proxy supports username/password authentication, then set
#   $SOCKS_USERNAME and $SOCKS_PASSWORD too.
...
# Special case:  If @BANNED_NETWORKS below includes "127" (localhost), and
#   $SOCKS_PROXY is set to something on localhost, then this script will still
#   allow itself to connect to $SOCKS_PROXY.  This is because most uses of
#   $SOCKS_PROXY are on localhost, where the connection is normally trusted.
#$SOCKS_PROXY= ':1080' ;
#$SOCKS_USERNAME= '' ;
#$SOCKS_PASSWORD= '' ;
...
#   administrator to create the database, tell them this database name.
...
#   do a 12-bit mask, we exclude the 16 172.x subnets this applies to--
#   inefficient, but works for now, until we support subnet masks.
...
@BANNED_NETWORKS= ('127', '192.168', '10', '169.254', '244.0.0',
...
#   nothing).
$MAX_REQUEST_SIZE= 16777216 ;  # that's 16 Meg to you and me
...
sub usage {
    print <<EOU ;
Usage:
  $ZERO  command  [ -n num_processes ]  [ -m max_requests ]  [ -p port ]
	  [ -f config_file_name ]  [ -q ]

Parameters:
  command
    ... where command is one of:
      install          install CGIProxy:
                         - run configuration wizard and menu
                         - create needed directories
                         - create configuration file
                         - install all required Perl (CPAN) modules
                         - create database
                         - copy script file into place
                         - add cron job to purge database 
      config           run configuration menu
      install-modules  install all required Perl (CPAN) modules
  ...
  -n, --num-processes  num_processes
    how many FastCGI processes will be started and maintained (default=$FCGI_NUM_PROCESSES)
  -m, --max-requests  max_requests
    limit how many requests a single FastCGI process can handle before
      restarting (default=$FCGI_MAX_REQUESTS_PER_PROCESS)
  -p, --port  port
    port number the embedded server should listen on (default=443)
  -f, --config-file  config_file_name
    use this configuration file instead of $PROXY_DIR/cgiproxy.conf
  -q, --quiet
    don't print FastCGI messages to the console
  -?, -h, --help
...
# Some badly-behaved module modifies $0, so save the original and use it
#   instead of $0 where needed.
$ZERO= $0 ;


# If $PROXY_DIR is relative, resolve it relative to the script owner's home
#   directory.  This has to be here rather than in init(), since we need
#   $PROXY_DIR to read the config file.
# On Windows, this sets $PROXY_DIR relative to the current user's home directory,
#   not the script owner's home directory.
# This block will only be run when installing, since the installation wizard
#   modifies the script to ensure $PROXY_DIR is set to an absolute directory.
# What a mess.
die "\$PROXY_DIR must not contain \"..\" .\n"  if $PROXY_DIR=~ /\.\./ ;
if ($^O eq 'MSWin32') {
    if (!($PROXY_DIR=~ m#^(?:[A-Za-z]:)?(?:/|\\)#)) {
	my $home_dir= "$ENV{HOMEDRIVE}$ENV{HOMEPATH}" ;
	die "Sorry, can't figure out home directory.\n"
	    unless $home_dir=~ m#^(?:[A-Za-z]:)?(?:/|\\)# ;
	$PROXY_DIR= File::Spec->catdir($home_dir, $PROXY_DIR) ;
    }
} else {
    if (!($PROXY_DIR=~ m#^/#)) {
	my $home_dir= (getpwuid( (stat($ZERO))[4] ))[7] ;
	$PROXY_DIR= File::Spec->catdir($home_dir, $PROXY_DIR) ;
    }
}
...
if (!$ENV{GATEWAY_INTERFACE}) {
    ...
    my($num_processes, $max_requests, $port_arg, $config_file, $quiet, $wants_help) ;
    GetOptions('num-processes|n=i' => \$num_processes,
	       'max-requests|m=i' => \$max_requests,
	       'port|p=i' => \$port_arg,
	       'config-file|f=s' => \$config_file,
	       'quiet|q' => \$quiet,
	       'help|h|?' => \$wants_help)  or die "bad options-- try '$ZERO -?' for help\n" ;
    usage()  if $wants_help or $cmd eq '' ;

    # Read in config file if it exists; otherwise, call in-script configuration.
    die "File $config_file does not exist.\n"
	if $cmd ne 'install' and $config_file ne '' and !-e $config_file ;
    $config_file||= File::Spec->catdir($PROXY_DIR, 'cgiproxy.conf') ;

    # Read config file if it exists, else run config().
    # When installing, start with all default values from config() .
    if (-e $config_file and $cmd ne 'install') {
	read_config_file($config_file) ;
    } else {
	config() ;
    }

    # Set this before either init() or one_run() .
	...
	# $quiet is messy to support, since FCGI and FCGI::ProcManager write a
	#   lot to STDERR, and we still want to let die() write to our original
	#   STDERR.  So, redirect STDERR to /dev/null while saving the original
	#   STDERR for die().  There may be a better approach, but the FCGI
	#   modules aren't well-documented.  :(
	if ($quiet) {
	    no warnings 'once' ;
	    ...
	    $SIG{__DIE__}= sub {
			       # pointless to "open() or die", since STDERR is still /dev/null
			       open(STDERR, '>&', 'OLDERR')  if $quiet and !$^S ;
			       die @_ ;
			   } ;
	}
	...
	$failed||= $@ ;
	die <<EOD  if $failed ;
For FastCGI, you need to install the FCGI and FCGI::ProcManager modules.
You can do this with "$ZERO install-modules", or by running the
installation wizard again and choosing a FastCGI installation.  You can
also install them yourself using the "cpan" utility or your system's
package manager.
EOD
	...
	# FCGI::ProcManager and FCGI modify the process name, but we set it
	#   with the pm_title parameter so that people can "killall nph-proxy.cgi"
	#   or whatever the script name is.
	my($zero)= $ZERO=~ m#([^/]*)$# ;
...
#	    warn $@ if $@ ;   # jsm-- should do anything else here?
	...
	$Net::SSLeay::ssl_version= 12 ;   # start with TLS 1.2, then try lower as needed
	$RUN_METHOD= 'embedded' ;
	...
	if ($LOCK_FH) {
	    ...
	    init() ;
	    if ($RUN_AS_GROUP_ID and $>==0) {
		$(= $)= $RUN_AS_GROUP_ID ;
		die "Can't change group ID to $RUN_AS_GROUP_ID: $!"  if $! ;
	    }
	    if ($RUN_AS_USER_ID and $>==0) {
		$<= $>= $RUN_AS_USER_ID ;
		die "Can't change user ID to $RUN_AS_USER_ID: $!"  if $! ;
	    }
    ...
    # Initialize the system as securely as possible.  How well we can do this
    #   depends on if we're the root user, script owner, in the script's group,
    #   or none of those.
    } elsif ($cmd eq 'init') {
	print <<EOM ;
The "init" command is no longer supported.  You probably want to run the
"install" command, which includes everything "init" used to do.
EOM


    } elsif ($cmd eq 'install') {
	my($run_method, $server_type, $server_conf_full, $server_root)= config_wizard() ;

	# Set all modes (permissions) we'll need.  Make them all as secure as
	#   possible:  if running as root, make restrictive and chown later;
	#   otherwise, everything will be owned by the current effective user
	#   ID, so set permissions to allow access to both effective user ID
	#   and $RUN_AS_USER_ID .
	my($old_umask)= umask(0) ;
	my($script_mode, $script_dir_mode, $proxy_dir_mode, $config_mode)=
	      ($>==0)                      ? (0700, 0700, 0700, 0600)
	    : ($>==$RUN_AS_USER_ID)        ? (0755, 0700, 0700, 0600)
	    : $)=~ /\b$RUN_AS_GROUP_ID\b/  ? (0755, 0750, 0750, 0640)
	    :                                (0755, 0755, 0755, 0644) ;

	# Create $PROXY_DIR/ .  make_path() handles its own errors.
	File::Path::make_path($PROXY_DIR, { mode => $proxy_dir_mode })  unless -d $PROXY_DIR ;

	# Set ownership of $PROXY_DIR:  If root, set both user and group,
	#   else if current user belongs to $RUN_AS_GROUP, set only group.
	if ($^O ne 'MSWin32') {
	    if ($>==0) {
		chown($RUN_AS_USER_ID, $RUN_AS_GROUP_ID, $PROXY_DIR)
		    or die "can't chown $RUN_AS_USER:$RUN_AS_GROUP $PROXY_DIR: $!" ;
	    } elsif ($)=~ /\b$RUN_AS_GROUP_ID\b/) {
		chown(-1, $RUN_AS_GROUP_ID, $PROXY_DIR)
		    or die "can't chown :$RUN_AS_GROUP $PROXY_DIR: $!" ;
	    }
	}

	$config_file= File::Spec->catdir($PROXY_DIR, 'cgiproxy.conf') ;
	write_config_file($config_file, $RUN_AS_USER_ID, $RUN_AS_GROUP_ID, $config_mode) ;

	umask($old_umask) ;

	init() ;

	install_modules($run_method) ;

	if ($DB_DRIVER ne '') {
	    my $rv= create_database_as_needed() ;
	    if ($rv==1) {
		print "Using existing database.\n\n" ;
	    } elsif ($rv==2) {
		print "Database \"$DB_NAME\" successfully created.\n\n" ;
	    } else {
	...
	# Copy script to one of these, depending on $run_method:  DocumentRoot/$SECRET_PATH/ ,
	#   ~/UserDir/$SECRET_PATH/ , or $PROXY_DIR/bin/ .
	my($installed_script, $url_guess)=
	    install_script($run_method, $server_type, $server_conf_full, $server_root,
		$script_mode, $script_dir_mode) ;

	# Add cron job to purge database if able, else tell user.
	add_cron_job_purge_db($installed_script)  if $DB_DRIVER ne '' ;

	print "*** CGIProxy successfully installed. ***\n\n" ;

	print <<EOM  if $url_guess ;
========================================================================
We think this is the URL of your new proxy:
  $url_guess

(Replace "localhost" with the domain name of your server.)
========================================================================

EOM


    } elsif ($cmd eq 'configure' or $cmd eq 'config') {
	config_menu() ;
	write_config_file($config_file) ;  # defaults owner/group/permissions to existing values


    } elsif ($cmd eq 'create-db') {
	init() ;

	die <<EOD unless -d $PROXY_DIR ;
The directory set in \$PROXY_DIR ($PROXY_DIR) doesn't exist-- you
need to run the \"install\" command to create it and cgiproxy.conf .
EOD
	die "You need to set \$DB_DRIVER before creating the database.\n"
	    unless $DB_DRIVER ne '' ;

	my $rv ;
	$rv= create_database_as_needed() ;
	if ($rv==1) {
...
# ... else is running as normal CGI or mod_perl script.
    ...
    if (!$HAS_INITED) {
	# Read in config file if it exists; otherwise, call in-script configuration.
	# Note that config file name is hard-coded here since we can't pass -f option.
	my $config_file= File::Spec->catdir($PROXY_DIR, 'cgiproxy.conf') ;
	if (-e $config_file) {
	    read_config_file($config_file) ;
	} else {
	    config() ;
...
exit unless $RUN_METHOD eq 'mod_perl' ;    # mod_perl scripts must not exit
...
#--------------------------------------------------------------------------
#   Set or adjust all globals that remain constant for all runs.
#--------------------------------------------------------------------------

# init() is somewhat overloaded-- it adjusts config variables, as well as
#   sets other globals for multiple runs.
sub init {


# First are a few variables that were in the config section, but aren't really
#   config variables.
...
# Set $RUN_AS_USER, $RUN_AS_USER_ID, $RUN_AS_GROUP, and $RUN_AS_GROUP_ID
#   based on settings of $RUN_AS_USER and $RUN_AS_GROUP, which can start as
#   any of empty, numeric IDs, or user/group names.
# They default to owner and group of this script file.
# Better to set $RUN_AS_USER and $RUN_AS_GROUP explicitly.
# Don't do this on Windows.
fix_run_as_user_group() if $^O ne 'MSWin32' ;


# Use local::lib if so configured.
# local::lib->import() seems to have a bug where it actually removes the
#   passed path from @INC, rather than adding it (?).  Running import()
#   twice seems to do what we need.  :/
if ($LOCAL_LIB_DIR ne '') {
    fix_local_lib_dir() ;   # ensure is absolute path
    push(@INC, File::Spec->catdir($LOCAL_LIB_DIR, qw(lib perl5))) ;   # to find local::lib
    eval {
	require local::lib ;
	local::lib->import($LOCAL_LIB_DIR) ;
	local::lib->import($LOCAL_LIB_DIR) ;
    } ;
    # ignore errors
}
...
$DB_FULLPATH= File::Spec->catfile($PROXY_DIR, 'sqlite', $DB_NAME)  if $DB_DRIVER eq 'SQLite' ;
...
if ($USER_IP_ADDRESS_TEST=~ m#^(https?)://([^/?:]*):?(\d*)(.*)#i) {
    my $port= $3 ne ''           ? $3
	    : lc($1) eq 'https'  ? 443
	    :                      80 ;
    @{$USER_IP_ADDRESS_TEST_H}{qw(scheme host port path socket open)}=
	(lc($1), lc($2), $port, $4, 'S_USERTEST', 0) ;
}
if ($DESTINATION_SERVER_TEST=~ m#^(https?)://([^/?:]*):?(\d*)(.*)#i) {
    my $port= $3 ne ''           ? $3
	    : lc($1) eq 'https'  ? 443
	    :                      80 ;
    @{$DESTINATION_SERVER_TEST_H}{qw(scheme host port path socket open)}=
	(lc($1), lc($2), $port, $4, 'S_DESTTEST', 0) ;
}



# If $RUNNING_ON_SSL_SERVER is '', then guess based on SERVER_PORT.
# Don't do this during installation.
$RUNNING_ON_SSL_SERVER= ($ENV_SERVER_PORT!=80)
    if $RUNNING_ON_SSL_SERVER eq '' and $RUN_METHOD ne '' ;
...
# Allow $SOCKS_PROXY to be set to just port number, with or without leading ":".
$SOCKS_PROXY= (substr($SOCKS_PROXY, 0, 1) eq ':')  ? "127.0.0.1$SOCKS_PROXY"
	    : $SOCKS_PROXY=~ /^\d+$/               ? "127.0.0.1:$SOCKS_PROXY"
	    :                                        $SOCKS_PROXY ;
...
$NO_CACHE_HEADERS= $MINIMIZE_CACHING
    ? "Cache-Control: no-store\015\012Pragma: no-cache\015\012"
    : '' ;
...
#--------------------------------------------------------------------------
#   Global constants are now set.
#--------------------------------------------------------------------------


#--------------------------------------------------------------------------
#   Do any initialization that is required for every run.
#--------------------------------------------------------------------------
...
# BUG IN PERL 5.24.1:  "reset" can cause a segmentation fault, untrappable by
#   eval/die .  Which other versions of Perl have this bug?  I've heard
#   conflicting reports.
if ($]==5.024001) {
    no strict 'refs' ;
    /^[a-z]/ && undef $$_  foreach sort keys %:: ;  # like "reset 'a-z'", but slower
} else {
    reset 'a-z' ;
}
...
$HTTP_VERSION= '1.1' unless $HTTP_VERSION=~ /^1\.[01]$/ ;
...
sub timeexit { goto ONE_RUN_EXIT }
...
# Generate the expected (?) network error for these URLs.
goto ONE_RUN_EXIT if $scheme eq 'chrome-extension' ;
...
if ($USER_IP_ADDRESS_TEST) {
	...
	$ok= $ENV{REMOTE_ADDR}=~ /[^\d.]/  ? 0  : `$USER_IP_ADDRESS_TEST '$ENV{REMOTE_ADDR}'` ;
    }
...
if ($DESTINATION_SERVER_TEST) {
	...
	# Die if $host contains either "'" or "\" .
	$ok= $host=~ /['\\]/  ? 0  : `$DESTINATION_SERVER_TEST '$host'` ;
    }
...
sub proxify_html {
    my($body_ref, $element_stack, $is_full_page, $no_exit_on_frameset)= @_ ;
    my(@out, $start, $comment, $script_block, $style_block, $svg_block, $mathml_block,
       $special_wo_html, $special_with_html, $cdata, $decl_bang, $decl_question, $tag,
       $body_pos, $html_pos, $head_pos, $first_script_pos, $out_start,
       $has_content, $title, $full_insertion, $body2, $old_url_start) ;
    ...
    # Some block-level HTML elements allow independent content inside them,
    #   including something like "<textarea><!--</textarea>" where the "<!--"
    #   is just text, so they have to be parsed as complete blocks to avoid
    #   starting an HTML comment.  These elements are <textarea>, <xmp>,
    #   <title>, <select>, <noscript>, <noembed>, <noframes>, and <iframe>.
    #   The last five can contain HTML that needs to be proxified, and the
    #   first three cannot.
    ...
    while ( $$body_ref=~ m{\G( (?> [^<]* ) (?> < (?![a-zA-Z/!\?]) [^<]* )* )
			     (?:(<!--(?=.*?-->).*?--\s*> | <!--(?!.*?-->).*?> )
			       ...
			       |(<svg\b.*?</svg\b.*?>)
			       |(<math\b.*?</math\b.*?>)
			       |(<(textarea|xmp|title)\b.*?</\8\b.*?>)
			       |(<(select|noscript|noembed|noframes|iframe)\b.*?</\10\b.*?>)
			       |(<!\[CDATA\[.*?\]\]>)
			       |(<![^>]*>?)
			       |(<\?[^>]*>?)
			       |(<[^>]*>?)
	...
	($start, $comment, $script_block, $style_block, $svg_block, $mathml_block,
	    $special_wo_html, $special_with_html, $cdata, $decl_bang, $decl_question, $tag)=
	...
	#   follow this block, in "elsif {}" blocks.
	...
	if ($tag) {
		...
		# Close <div> block that surrounds entire original page, if needed.
	    ...
	    #   But note that $doing_insert_here implies !$is_in_frame .
		...
		&return_frame_doc(&wrap_proxy_encode($URL), $title)
		    if $doing_insert_here && !$no_exit_on_frameset ;
	    }

	    # Note that $full_insertion implies $is_full_page .
	    push(@out, proxify_html_tag($tag, $element_stack, $body_ref,
					$full_insertion  ? \$full_insertion  : undef)) ;




	# $tag processing done.  Now, handle the other main cases-- comments,
	#   <script> blocks, <style> blocks, <!...> declarations, etc.


	# Handle comments of both the <!--...--> and <!--...> varieties.
	# For now, don't handle XML namespaces when proxifying comments--
	#   conditional comments don't make well-defined XML.
	} elsif ($comment) {
		...
		$condition= &proxify_block($condition, $default_script_type)
		    if $PROXIFY_SCRIPTS ;
		$contents=  &proxify_html(\$contents, undef, 0, $no_exit_on_frameset) ;
		...
		$contents=  &proxify_html(\$contents, undef, 0, 1) ;
	...
	#   a Perl-to-JavaScript translation of this block.
	    ...
	    $tag= &proxify_html_tag($tag, $element_stack) ;
	    shift(@$element_stack)  if $element_stack ;

	    # Remove <script> block if the opening tag was removed.
		...
		$type= $language=~ /javascript|ecmascript|livescript|jscript/i
		     ...
		     : $language=~ /vbs/i        ? 'application/x-vbscript'
		...
		next unless $ALLOW_UNPROXIFIED_SCRIPTS ;
	    }
	...
	# Used for <textarea>, <xmp>, and <title>.
	} elsif ($special_wo_html) {
	    my($tag, $tag_name, $contents, $end_tag) ;

	    ($tag, $tag_name, $contents, $end_tag)=
		$special_wo_html=~ m#^(<\s*(\w+)\b[^>]*>)(.*?)(<\s*/\2\b.*?>)$#si ;

	    # Handle page titles here.  This includes extracting one into $title,
	    #   and clearing it if $REMOVE_TITLES is set.  Slightly hacky.
	    if (lc($tag_name) eq 'title') {
		$contents= ''  if $REMOVE_TITLES ;
		$title= $contents ;
	    }
	    ...
	    push(@out, $tag, $contents, $end_tag) ;



	# Used for <select>, <noscript>, <noembed>, <noframes>, and <iframe>.
	} elsif ($special_with_html) {
	    my($tag, $tag_name, $contents, $end_tag) ;

	    ($tag, $tag_name, $contents, $end_tag)=
		$special_with_html=~ m#^(<\s*(\w+)\b[^>]*>)(.*?)(<\s*/\2\b.*?>)$#si ;
	    ...
	    #   style, etc.
	    if ($scripts_are_banned_here and lc($tag_name) eq 'noscript') {
	    ...
	    $tag= &proxify_html_tag($tag, $element_stack) ;
	    $contents= &proxify_html(\$contents, $element_stack, 0) ;
	    shift(@$element_stack)  if $element_stack ;
	    ...
	    my($tag, $contents, $end_tag) ;
	    ($tag, $contents, $end_tag)=
		$svg_block=~ m#^(<\s*svg\b[^>]*>)(.*?)(<\s*/svg\b.*?>)#si ;

	    next ;   # jsm-- until we proxify SVG

	    $tag= &proxify_html_tag($tag, $element_stack) ;
	    $contents= &proxify_block($contents, 'image/svg+xml') ;
	    shift(@$element_stack)  if $element_stack ;
	    ...
	    my($tag, $contents, $end_tag) ;
	    ($tag, $contents, $end_tag)=
		$mathml_block=~ m#^(<\s*math\b[^>]*>)(.*?)(<\s*/math\b.*?>)#si ;

	    next ;   # jsm-- until we proxify MathML

	    $tag= &proxify_html_tag($tag, $element_stack) ;
	    $contents= &proxify_block($contents, 'application/mathml+xml') ;
	    shift(@$element_stack)  if $element_stack ;
	...
	splice(@out, $insert_pos, 0, <<EOS)  if $doing_insert_here && $has_content ;
<style>
...
EOS
	...
	# Optimize all pages and frames for mobile.
	splice(@out, $insert_pos, 0,
...
sub proxify_html_part {
	...
	push(@out, &proxify_html(\$block, $element_stack)) ;
...
sub transmit_html_in_parts {
    ...
    my($buf, $length, $numread, $thisread, $out, $in, $element_stack) ;
	...
	while ($chunk_size= hex(<$S>) ) {
	    ...
	    ($out, $buf)= &proxify_html_part(\$buf, $element_stack) ;
...
sub proxify_html_tag {
    my($tag, $element_stack, $body_ref, $full_insertion)= @_ ;
    ...
    if (substr($tag_name, 0, 1) eq '/') {
	# Pop $element_stack if we got an end tag.  Verify that the end
	#   tag matches the last open element, which should be true for
	#   XML resources.  Note that $element_stack being defined implies
	#   that we have an XML resource.
	if ($element_stack) {
	    &HTMLdie("Bad XML: <$tag_name> does not match <$element_stack->[0][0]>")
		unless substr($tag_name, 1) eq $element_stack->[0][0] ;
	    shift(@$element_stack) ;
	}
	return $tag ;
    }

    # This is used both when handling $element_stack and when rebuilding $tag.
    my $end_slash= substr($tag, -2, 1) eq '/'  ? ' /'  : '' ;
...
#	while ($attrs=~ /([A-Za-z][\w.:-]*)\s*(?:=\s*(?:"([^">]*)"?|'([^'>]*)'?|([^'"][^\s>]*)))?/g ) {
    ...
    # Add the element and any namespaces found to $element_stack .
    # Note that namespaces are in effect for the tag that contains them.
    if ($element_stack) {
	unshift(@$element_stack, [ $tag_name ]) ;
	foreach (grep { /^xmlns\b/i } keys %attr) {
	    # '' means a default namespace
	    my $nsname= /:/  ? (split(/:/, $_, 2))[1]  : '' ;
	    push(@{$element_stack->[0]}, [ $nsname, $attr{$_} ]) ;
	}
    }


    # Pass tag through if it has no attributes, or if it doesn't parse
    #   above (which would make %attr empty).
    if (!%attr) {
	shift(@$element_stack)  if $element_stack and $end_slash ;
	return $tag ;
    }


    # Remove all VBSCRIPT <script> blocks.
    if ($tag_name eq 'script' and $attr{language}=~ /vbs/i) {
	shift(@$element_stack)  if $element_stack and $end_slash ;
	return undef ;
    }

    # Remove all "language=vbs[cript]" attributes from any tag.
    delete $attr{language}, $rebuild= 1  if $attr{language}=~ /vbs/i ;
    ...
    #   HTMLescape()'ing.  We can revisit this later if it becomes an
    #   issue.
	...
	foreach (grep(/^on/i, keys %attr)) {
	    $rebuild= 1 ;
	    delete $attr{$_}, next if $attr{$_}=~ /^\s*(?:vbs|vbscript)\s*:/i ;
	    $attr{$_}= &proxify_block($attr{$_}, $type) ;
	}
    }
	...
	$$full_insertion=~ s/(\bid="_proxy_css_main_div\")/$1 class="$attr{class}"/
	    if $full_insertion && defined($attr{class}) ;
	...
	$attr{src}=      &full_url_by_frame($attr{src}, 1), $rebuild=1 if defined($attr{src}) ;
	$attr{longdesc}= &full_url($attr{longdesc}),        $rebuild=1 if defined($attr{longdesc}) ;
	$attr{srcdoc}=   &proxify_html($attr{srcdoc}, $element_stack, 0),
							    $rebuild=1 if defined($attr{srcdoc}) ;
	...
	# <button href> is not standard, but some sites (e.g. youtube.com) use it.
	$attr{href}=       &full_url($attr{href}),       $rebuild=1  if defined($attr{href}) ;
    ...
    # Obsolete, but still supported by Firefox.
    } elsif ($tag_name eq 'isindex') {
	$attr{action}= &full_url($attr{action}),            $rebuild=1 if defined($attr{action}) ;
    ...
    # proxify these attributes for all unknown tags.
    } else {
	$attr{action}=  &full_url($attr{action}),     $rebuild=1  if defined($attr{action}) ;
	$attr{formaction}= &full_url($attr{formaction}), $rebuild=1  if defined($attr{formaction}) ;
	$attr{href}=    &full_url($attr{href}),       $rebuild=1  if defined($attr{href}) ;
...
sub full_url {
    ...
    # Remove all "vbs:" and "vbscript:" URLs.
    return undef if $uri_ref=~ /^\s*(vbs|vbscript)\s*:/i ;
	...
	if ($type eq 'text/html' or $type=~ /^$TYPES_TO_HANDLE_REGEX$/io) {
	    ...
	    # jsm-- should namespaces be handled here?
    ...
    # Note that browsers (non-standardly) support "\" in URLs to mean the same
    #   as "/", so handle those too.
    1 while $uri=~ s#^[/\\]\.\.?[/\\]#/# ;
    ...
    $absurl= sanitize_url_path($absurl) ;
...
# Collapse or remove "." and ".." path segments in an absolute URL.
# Could be more efficient, but required to avoid path-traversal exploits.
sub sanitize_url_path {
    my($absurl)= @_ ;

    # Note that $scheme_authority gets initial "/" of path.
    my($scheme_authority, $path, $rest)= $absurl=~ m!^([\w+.-]+://[^/\\?#]*[/\\]?)([^?#]*)(.*)$! ;
    my @ret ;
    my $ends_with_slash= $path=~ m#[/\\]$# ;   # needed since split will lose it otherwise
    foreach (split(m#[/\\]#, $path)) {
	next if $_ eq '.' ;
	pop(@ret), next if $_ eq '..' ;
	push(@ret, $_) ;
    }
    return $scheme_authority . join('/', @ret) . ($ends_with_slash ? '/' : '') . $rest ;
}
...
sub same_origin {
    ...
    return ($s1 eq $s2) && ($h1 eq $h2) && ($p1 eq $p2) ;
}
...
#	    } until (/^(\015\012|\012)$/) || eof(S) ; # lines end w/ LF or CRLF
	...
	#   unwanted directives and add the "no-store" directive, and add back
	...
	#   extensions for the same reason.  This effectively means preserve
	#   only "no-transform" if it exists (it has no parameters), and add
	#   "no-store".
	# Note that there was previous confusion about no-cache vs. no-store.
	#   no-cache is somewhat misnamed-- it means the client can cache it
	#   but must revalidate every time.  no-store is what we want here,
	#   to request no caching at all.
	...
	# jsm-- not satisfied with this... what about non-XHTML XML?
	$is_xhtml= ($content_type eq 'text/xml')
		|| ($content_type eq 'application/xml')
		|| ($content_type eq 'application/xhtml+xml') ;


	# Set $is_html if headers indicate HTML response.
	# Question: are there any other HTML-like MIME types, including x-... ?
	$is_html= 1  if   $content_type eq 'text/html' or $is_xhtml ;
	...
	if ($ENV{'REQUEST_METHOD'} eq 'HEAD') {
		...
		# Pass through response if chunked.
		    ...
		    # Unfortunately, nginx doesn't handle chunked responses
		    #   correctly.  :(  We can either a) store the entire
		    #   chunked response before sending anything, or b) send
		    #   along each chunk as we get it, with no size indication,
		    #   and ignore any trailers.  Both approaches are problematic,
		    #   but for now we'll go with the second approach.
		    if ($RUN_METHOD eq 'fastcgi' and $ENV{SERVER_SOFTWARE}=~ /^nginx\b/i) {
			# Remove Transfer-Encoding: header.
			$headers=~ s/^Transfer-Encoding:[ \t]*chunked\b.*\015?\012//mi ;
			print $STDOUT $status, $headers ;
			...
			while ($hex_size= <S>) {
			    ...
			    scalar <S> ;    # clear CRLF after chunk
			}
			# Get footers.
			while (<S>) {
			    last if /^(\015\012|\012)/  || $_ eq '' ;   # lines end w/ LF or CRLF
		    ...
		    # This is the normal behavior, for every server but nginx.
		    } else {
			print $STDOUT $status, $headers ;
			while ($hex_size= <S>) {
		...
		if ($transmit_in_parts) {
		    ...
		    $body= &proxify_html(\$body, undef, 1) ;
...
# was:  ($rcode, @r)= &ftp_command("LIST $path\015\012", '125|150') ;
	...
	$body= &proxify_html(\$body, undef, 1)  if $is_html ;
...
sub ftp_dirfix {
    ...
    $body=~ s/\0//g ;   # prevent potential exploit
...
#	next unless $#f>=8 ;
	...
	if ($is_dir || $is_link) {
	    ...
	    ($size, $size_type)= (HTMLescape($f[4]), 'bytes') ;
	...
	$newbody.=
	    sprintf("  <a href=\"%s\">%s</a>%s %5s %-5s %3s %2s %5s  %s\012",
			   ...
			   $size, $size_type,
			   HTMLescape($f[5]), HTMLescape($f[6]), HTMLescape($f[7]),
			   $file_type) ;
    }
...
<pre>
EOR
    $response= sprintf($response, $dir, $host) . $r . "</pre>\n" . footer() ;
...
sub create_server_lock {
    ...
    my $lock_path= File::Spec->catfile($PROXY_DIR, $lock_file) ;
    open($LOCK, (-s $lock_path  ? '+<'  : '+>'), $lock_path) || die "open: $!" ;
...
sub double_fork_daemon {
    ...
    setsid() || die "setsid: $!"  unless $^O eq 'MSWin32' ;
...
#    unshift(@headers, "Content-Security-Policy: default-src $THIS_SCRIPT_URL/ 'unsafe-inline' 'unsafe-eval' ; img-src $THIS_SCRIPT_URL/ data: ; form-action $THIS_SCRIPT_URL/ ; base-uri $THIS_SCRIPT_URL/\015\012") ;
    ...
    my $csp_header_value= "default-src $csp_source 'unsafe-inline' 'unsafe-eval' ; style-src $csp_source 'unsafe-inline' data: ; img-src $csp_source data: ; font-src $csp_source data: ; media-src $csp_source blob:" ;
...
sub new_header_value {
    ...
    #   flag segment.
    # Regarding $NOT_RUNNING_AS_NPH and Location: headers:  Technically, the
    #   Location: header has a slightly different meaning as a CGI header than
    #   as an HTTP header.  They are almost the same, but a CGI Location: header
    #   can start with "/" to instruct the server to return another local resource
    #   in response to the request, while an HTTP Location: header must be an
    #   absolute URL.  Here, we only receive HTTP Location: headers and not
    #   CGI Location: headers, so they *should* always be absolute URLs.
    #   However, in the event that a malicious server sends a relative URL,
    ...
    $value=~ s/(;(?:\s*URL\s*=)?)\s*(?>['"]?)(\S*)\2/ $1 . &full_url($2) /ie,   return $value
	if $name eq 'refresh' ;
...
return undef ;   # we don't support this header yet
    ...
    # We don't support Alt-Svc: .
    return undef if $name eq 'alt-svc' ;
...
sub parse_csp_header {
    ...
    # Don't autovivify frame-src if child-src doesn't exist.
    $new_policy->{'frame-src'}||= $new_policy->{'child-src'} if $new_policy->{'child-src'} ;
...
sub csp_is_supported {
    ...
    return $1>=7   if $ENV{HTTP_USER_AGENT}=~ /\bSafari\// and $ENV{HTTP_USER_AGENT}=~ /\bVersion\/(\d+)/ ;
    return $1>=12  if $ENV{HTTP_USER_AGENT}=~ /\bEdge\/(\d+)/ ;
...
sub newsocketto {
	...
	# If using $SOCKS_PROXY, check destination addresses for @BANNED_NETWORKS .
	# This is imperfect in that it will fail if any of @addr_destination is bad.
	if ($SOCKS_PROXY and @BANNED_NETWORKS) {
	    my($err, @addr_destination)= $_[1]=~ /^[\dA-Fa-f:]*[\d.]*$/
		? getaddrinfo($_[1], $_[2], { flags => AI_NUMERICHOST(), socktype => SOCK_STREAM })
		: getaddrinfo($_[1], $_[2], { socktype => SOCK_STREAM }) ;
	    &HTMLdie(["Couldn't find address for %s: %s", $_[1], $!]) if $err==EAI_NONAME() ;
	    &HTMLdie(["Error looking up address for %s: %s", $_[1], $!]) if $err ;

	    foreach my $addr (@addr_destination) {
		if ($addr->{family}==AF_INET) {
		    # NIx_NOSERV() didn't exist before Socket version 1.96, so test for it.
		    my($err, $ipaddr) = defined(&NIx_NOSERV)
			? getnameinfo($addr->{addr}, NI_NUMERICHOST(), NIx_NOSERV())
			: getnameinfo($addr->{addr}, NI_NUMERICHOST()) ;
		    for (@BANNED_NETWORKS) {
			&banned_server_die() if $ipaddr=~ /^\Q$_\E(?:\.|$)/ ;
		    }
		} else {
		    # warn "\@BANNED_NETWORKS not supported for IPv6 yet.\n";
		}
	    }
	}
	...
	foreach my $addr (@addr) {
		...
		if (@BANNED_NETWORKS) {
			...
			: getnameinfo($addr->{addr}, NI_NUMERICHOST()) ;
		    for (@BANNED_NETWORKS) {
			next if $SOCKS_PROXY and $_ eq '127' ;   # special case
			&banned_server_die() if $ipaddr=~ /^\Q$_\E(?:\.|$)/ ;
		    }
		}
	    } else {
	...
	# Support @BANNED_NETWORKS, but allow connection to a SOCKS proxy on localhost.
	if (@BANNED_NETWORKS) {
	    my $ipaddr= inet_ntoa($hostaddr) ;
	    if ($SOCKS_PROXY) {
		my $hostaddr_destination= inet_aton($_[1])
		    or &HTMLdie(["Couldn't find address for %s: %s", $_[1], $!]) ;
		my $ipaddr_destination= inet_ntoa($hostaddr_destination) ;
		for (@BANNED_NETWORKS) {
		    # Check both SOCKS proxy and destination IP address.
		    &banned_server_die()  if $ipaddr_destination=~ /^\Q$_\E(?:\.|$)/ ;
		    next if $_ eq '127' ;  # special case-- don't block SOCKS proxy on localhost
		    &banned_server_die()  if $ipaddr=~ /^\Q$_\E(?:\.|$)/ ;
		}
	    } else {
		for (@BANNED_NETWORKS) {
...
sub date_is_after {
	...
	$d1[1]= $UN_MONTH{lc($d1[1])}-1 ;   # %UN_MONTH is 1-based
	$d1[2]-= 1900 ;
    }
    if ($date2=~ /^\d+$/) {
...
# This routine expects a pointer to a hash containing "scheme", "host", "port", "socket",
...
#   and authentication.  Eventually, http_get() may be restructured to be more
#   modular and support what we need here.
# This is partially copied from http_get().  For more commenting, see that
#   routine, in the similar sections as below.
# jsm-- must retest this.
sub http_get2 {
    my($c, $request_uri)= @_ ;
	...
	if (!$c->{'open'} || !select(undef, $win, undef, 0)) {
	    if ($c->{scheme} eq 'http') {
		return undef unless newsocketto_simple($c->{'socket'}, $c->{host}, $c->{port}) ;
	    } else {
		eval { require Net::SSLeay } ;
		die "Can't load Net::SSLeay module: $@\n"  if $@ ;
		$Net::SSLeay::ssl_version= 12 ;   # start with TLS 1.2, then try lower as needed
		return undef unless newsocketto_simple($c->{plain_handle}, $c->{host}, $c->{port}) ;
		select((select($c->{plain_handle}), $|= 1)[0]) ;   # autoflush
		$c->{ssl_obj}= tie(*$c->{'socket'}, 'SSL_Handle', \*$c->{plain_handle}) ;
		# Support the TLS SNI extension.  Thanks to Carsten Gaebler for the patch.
		Net::SSLeay::set_tlsext_host_name($c->{ssl_obj}{SSL}, 'localhost')
		if defined &Net::SSLeay::set_tlsext_host_name ;
		Net::SSLeay::connect($c->{ssl_obj}{SSL})  or die "Can't SSL connect: $!\n" ;
	    }
	    $s= $c->{'socket'} ;
	    $c->{'open'}= 1 ;
	...
	close($s) ;
	untie($c->{'socket'}), close($c->{plain_handle}) if $c->{scheme} eq 'https'  ;
	$c->{'open'}= 0 ;
    }
    ...
    if ($c->{'open'} and ($headers=~ /^Connection:.*\bclose\b/mi or $status=~ m#^HTTP/1\.0#)) {
	close($s) ;
	untie($c->{'socket'}), close($c->{plain_handle}) if $c->{scheme} eq 'https'  ;
	$c->{'open'}= 0 ;
    }
...
sub full_insertion {
	...
	if ($ANONYMIZE_INSERTION) {
	    ...
	    &fix_base_vars ;
	    $insertion= &proxify_html(\$CUSTOM_INSERTION, undef, 0) ;
	} else {
...
sub show_start_form {
    ...
    my $cache_control= $MINIMIZE_CACHING
	? 'no-store, no-transform'
	: 'no-cache, no-transform, private, max-age=31536000' ;   # expires after one year

    # If client sent If-Modified-Since: header and this script hasn't been modified
    #   since the date indicated, then send 304 response.
    if ($ENV{HTTP_IF_MODIFIED_SINCE} and date_is_after($ENV{HTTP_IF_MODIFIED_SINCE}, (stat($ZERO))[9])) {
	print $STDOUT "$HTTP_1_X 304 Not Modified\015\012",
		      "${session_cookies}Date: $date_header\015\012",
		      "Cache-Control: $cache_control\015\012",
		      "\015\012" ;
	die "exiting" ;
    }
	...
	$onload= qq( onload="document._proxy_jslib_URLform.URL.focus() ; if (document._proxy_jslib_URLform.URL.value.match(/^$encode_prefix/)) document._proxy_jslib_URLform.URL.value= _proxy_jslib_wrap_proxy_decode(document._proxy_jslib_URLform.URL.value.replace(/$encode_prefix/, ''))") ;
    } else {
	$jslib_block= $onsubmit= '' ;
	$onload= ' onload="document._proxy_jslib_URLform.URL.focus()"' ;
    }
...
<form name="_proxy_jslib_URLform" action="%s" method="%s"%s>
...
EOR
    ...
    print $STDOUT <<EOR . $response ;
$HTTP_1_X 200 OK
${session_cookies}Date: $date_header
...
<form name="_proxy_jslib_URLform" action="%s" method="%s" target="_top"%s%s>
<center>
%s
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>Enter username and password for %s at %s</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>WARNING: Entering non-anonymous area!</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>WARNING: SSL not supported, entering non-anonymous area!</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>Compressed content not supported, but was sent by server.</title>
</head>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>The proxy can't access that server, sorry.</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>You are not allowed to use this proxy, sorry.</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>Proxy cannot loop back through itself</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>Retrieval of secure URLs through a non-secure proxy is forbidden.</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>Script content blocked</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>Proxy cannot forward non-text files</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>Page uses UTF-16 encoding, which is unsupported by this version
      of Perl</title>
</head>
<body>
...
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>Page has malformed Unicode</title>
</head>
<body>
...
in question.
EOR
    $response= sprintf($response, $dir, HTMLescape($charset)) . footer() ;
    eval { $response= encode('utf-8', $response) } ;
...
sub HTMLdie {
    ...
    # Don't use HTML if not serving Web pages, like with "install" command.
    die "$msg\n" unless $RUN_METHOD ;
    ...
    $HTTP_1_X=  $NOT_RUNNING_AS_NPH   ? 'Status:'   : "HTTP/1.1"
	if $HTTP_1_X eq '' ;
  ...
  ELEMENT:
	...
	if (ref($in->[$i])) {
		...
		if (@{$in->[$i+2]}==2 and $in->[$i+2][0]=~ /^\d+$/) {
			...
			if ($prefix ne '') {
			    ...
			    $term_so_far= [ '_proxy_jslib_handle((', $term_so_far, '), (', $sub_expr,
					    '), void 0, ', $next_is_paren, ", $in_new_statement)", $in->[$i-1] ] ;
			}
		...
		if (@$block) {   # {} block is not empty
			...
			last if $end >= @$block ;
			if (($block->[$end+2] ne ':') and ($block->[$end] eq 'get' or $block->[$end] eq 'set')) {
			    return undef unless $block->[$end]=~ /^(?:$RE_JS_IDENTIFIER_START|['"\d]|\.\d)/o ;
			} else {
			    return undef
				unless $block->[$end+2] eq ':'
				    and $block->[$end]=~ /^(?:$RE_JS_IDENTIFIER_START|['"\d]|\.\d)/o ;
			    $block->[$end]=~ s/^_proxy(\d*)_/'_proxy'.($1+1).'_'/e ;
			    push(@$term_so_far, @$block[$end..$end+3]) ;
			    $end+= 4 ;
			}
			$start= $end ;
	    ...
	    } elsif ($token eq 'function' or
		     (!@$term_so_far and ($token eq 'get' or $token eq 'set')) )
	    {
		push(@$ret, $prefix, $term_so_far) ;
		$prefix= '' ;
		$term_so_far= [ $token, $in->[$i+1] ] ;
	    ...
	    } elsif ($token=~ /^(?:LoadMovie|String|URL|action|addImport|appendChild|assign|background|backgroundImage|baseURI|body|cite|close|codeBase|content|contentDocument|contentWindow|cookie|createElement|cssText|currentSrc|cursor|data|defineProperties|defineProperty|documentURI|domain|eval|execCommand|execScript|fetch|formAction|frames|get|getAttribute|getAttributeNS|getComputedStyle|getElementById|getElementsByTagName|getOwnPropertyDescriptor|getPropertyValue|host|hostname|href|innerHTML|insertAdjacentHTML|insertBefore|insertRule|listStyleImage|load|localStorage|location|longDesc|lowsrc|navigate|navigator|newURL|nodeValue|oldURL|open|opener|origin|outerHTML|outerText|parent|parentNode|pathname|port|postMessage|poster|profile|protocol|pushState|querySelector|querySelectorAll|referrer|removeChild|replaceChild|replaceState|responseURL|search|send|sessionStorage|set|setAttribute|setAttributeNode|setAttributeNodeNS|setAttributeNS|setInterval|setNamedItem|setNamedItemNS|setProperty|setRequestHeader|setStringValue|setTimeout|showModalDialog|showModelessDialog|src|srcdoc|srcset|style|text|textContent|toLocaleString|toString|top|url|useMap|value|withCredentials|write|writeln)$/)
	    {
		...
		if ($i>=2 and
		    ($in->[$i-2] eq '}' or
		     ($in->[$i-1]=~ $RE_JS_LINE_TERMINATOR and
		      $in->[$i-2]=~ m#^(?:\)|\]|\+\+|--)$|
		...
		if (!$had_dot and !@$term_so_far and $in->[$i+2] eq ':' and ($i<2 or
		    ...
		    and $token!~ /^(?:location|open|setInterval|setTimeout|frames|parent|top|opener|execScript|navigate|navigator|showModalDialog|showModelessDialog|parentWindow|String|getComputedStyle|fetch)$/)
		{
		...
		} elsif ($i>2 and ($in->[$i-2] eq '{' or $in->[$i-2] eq ',')
			 and ($token eq 'get' or $token eq 'set' or $in->[$i+2] eq ':') )
		{
		    push(@$ret, $prefix, $term_so_far, $token, $in->[$i+1]) ;
		    $prefix= '' ;
		    $term_so_far= [] ;
	    ...
	    # Unfortunately, twitter.com uses "default" as a property name and
	    #   browsers let them get away with it, so only treat it as a keyword
	    #   when it's followed by ":".
	    } elsif ($token=~ /^(?:abstract|boolean|byte|case|char|class|const|debugger|delete|do|else|enum|export|extends|final|finally|float|goto|implements|in|instanceof|int|interface|long|native|package|private|protected|return|short|static|synchronized|throw|throws|transient|try|typeof|void|volatile)$/
		     or ($token eq 'default' and $in->[$i+2] eq ':') )
	    {
...
sub parse_js {
	...
	# Convert \uxxxx sequences in identifiers into characters
	$token=~ s/\\u([0-9A-Fa-f]{4})/ chr(hex($1)) /ge  if $token=~ /^$RE_JS_IDENTIFIER_START/o ;
...
var _proxy_jslib_handle_properties= 'eval insertAdjacentHTML setAttribute setAttributeNS setAttributeNode setAttributeNodeNS getAttribute getAttributeNS value insertRule innerHTML outerHTML outerText src currentSrc href background lowsrc action formAction useMap longDesc cite codeBase location poster data srcdoc srcset open write writeln URL url newURL oldURL documentURI referrer baseURI body parentNode toString toLocaleString String setInterval setTimeout cookie domain frames parent top opener protocol host hostname port pathname search setStringValue setProperty setNamedItem setNamedItemNS load execScript navigate navigator showModalDialog showModelessDialog addImport execCommand LoadMovie getElementById getElementsByTagName appendChild replaceChild insertBefore removeChild createElement text close origin postMessage pushState replaceState localStorage sessionStorage querySelector querySelectorAll send setRequestHeader getPropertyValue withCredentials responseURL assign contentDocument contentWindow defineProperties defineProperty getOwnPropertyDescriptor get set nodeValue textContent style getComputedStyle fetch'.split(/\s+/) ;

var _proxy_jslib_assign_properties= 'background src href lowsrc action useMap longDesc cite codeBase location poster data srcdoc srcset profile cssText innerHTML outerHTML outerText nodeValue protocol host hostname port pathname search cookie domain value backgroundImage content cursor listStyle listStyleImage text textContent withCredentials documentURI style'.split(/\s+/) ;
...
var _proxy_jslib_ORIGINAL_OBJECT_toString= Object.prototype.toString ;
...
var _proxy_jslib_ORIGINAL_STRING_match= String.prototype.match ;
...
var _proxy_jslib_ORIGINAL_HISTORY_pushState= History.prototype.pushState ;
var _proxy_jslib_ORIGINAL_HISTORY_replaceState= History.prototype.replaceState ;


// This is the only constructor we rewrite, and it needs special handling in
//   several places to block prototype changes to it.  Messy.  :P
var _proxy_jslib_String_constructor= function(s) {
...
// This will catch automatic invocations of toString() on these classes, such
//   as with alert() or string arithmetic.  Not perfect.
var _proxy_jslib_modified_prototypes ;
try {
    HTMLAnchorElement.prototype.toString= HTMLAreaElement.prototype.toString=
	function() {
	    return _proxy_jslib_parse_full_url(this.href)[3] ;
	} ;
    _proxy_jslib_modified_prototypes= 1 ;
} catch(e) {}


var _proxy_jslib_supports_proxy_class= _proxy_jslib_is_native_or_host_class(Proxy) ;

// as of July 2017, there's a bug in Chrome where a Proxy calling a native method
//   of its target object will throw an "Illegal Invocation" error, so test for it.
try {
    new Proxy(window, {}).getComputedStyle(document.body) ;
} catch(e) {
    _proxy_jslib_supports_proxy_class= false ;
}


var _proxy_jslib_style_handler= {
    get: function(o, property) {
	     return _proxy_jslib_proxify_css(o[property], 1) ;
	 },
    set: function(o, property, value) {
	     try {
		 o[property]= _proxy_jslib_proxify_css(value) ;
		 return true ;
	     } catch(e) {
		 return false ;
	     }
...
function _proxy_jslib_onload() {
	...
	top._proxy_jslib_insertion_frame.document._proxy_jslib_URLform.URL.value= _proxy_jslib_URL ;
...
function _proxy_jslib_handle (o, property, cur_val, calls_now, in_new_statement) {
    ...
    var match ;
    if (property && (match= _proxy_jslib_ORIGINAL_STRING_match.call(property.toString(), /^_proxy(\d*)_/)))
	property= '_proxy' + (match[1]-0+1) + property.toString().replace(/^_proxy(\d*)/, '') ;
    ...
    // unproxify style properties, if they're accessed using style[property]
    // direct property access like "o.backgroundImage" must be handled elsewhere
    if (_proxy_jslib_instanceof(o, 'CSSStyleDeclaration') && (typeof o[property]!='function'))
	return _proxy_jslib_supports_proxy_class  ? o[property]  : _proxy_jslib_proxify_css(o[property], 1) ;


    // guess when the window object is implied; this only matters with Window's
    //   properties that we handle below
    if ((o===null)  && (typeof(property)=='string') && _proxy_jslib_ORIGINAL_STRING_match.call(property, /^(location|open|setInterval|setTimeout|frames|parent|top|opener|execScript|navigate|navigator|showModalDialog|showModelessDialog|parentWindow|String|postMessage|localStorage|sessionStorage|getComputedStyle|fetch)$/) && (window[property]===cur_val)) o= window ;
...
//if (u=='') alert('in handle, first switch; typeof, o, property, u, caller=['+typeof(o)+']['+o+']['+property+']['+u+']\n['+arguments.callee.caller.caller+']') ;
	...
	case 'background':
	    ...
	    } else if (_proxy_jslib_instanceof(o, 'CSS2Properties') || _proxy_jslib_instanceof(o, 'CSSStyleDeclaration'))
	    {
		return _proxy_jslib_supports_proxy_class
		    ? o[property]  : _proxy_jslib_proxify_css(o[property], 1) ;
	    }


	case 'srcdoc':
	    if (_proxy_jslib_instanceof(o, 'HTMLElement') && (o.tagName.toLowerCase()=='iframe')) {
		return _proxy_jslib_proxify_html(o.srcdoc, o.contentDocument, true)[0] ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'fetch':
	    if (_proxy_jslib_instanceof(o, 'Window') || _proxy_jslib_instanceof(o, 'WorkerGlobalScope')) {
		return function (input, init) {
			   if ((typeof input=='string') || _proxy_jslib_instanceof(input, 'String'))
			       input= _proxy_jslib_full_url(input) ;
			   return o[property](input, init) ;
		       } ;
	    } else {
		return _handle_default() ;
	    }
...
//alert('ending Document.close()') ;
	...
	case 'url':
	    if (_proxy_jslib_instanceof(o, 'EventSource') || _proxy_jslib_instanceof(o, 'Request')) {
	...
	case 'createElement':
		...
		return function (localName) {
			   ...
			   if (localName.toLowerCase()=='iframe')
				   ...
				   ret.contentDocument.write(_proxy_jslib_iframe_init_html()) ;
				   ret.contentDocument._proxy_jslib_has_jslib= 1 ;
			       }
			   return ret ;
		       } ;
	    } else {
	...
	case 'textContent':
		...
		if (o.nodeName.match(/^script$/i)) {
		    var type= o.type || _proxy_jslib_default_script_type ;
		    return _proxy_jslib_proxify_block(o[property], type,
			_proxy_jslib_ALLOW_UNPROXIFIED_SCRIPTS, 1) ;
	    ...
	    } else if (property=='textContent' && _proxy_jslib_instanceof(o, 'Attr')) {
	...
	// Element.setAttributeNS() isn't well-documented, but any namespace is
	//   reflected in both the namespace and name parameters, so we can get
	//   away with using _proxy_jslib_proxify_attribute(), which doesn't
	//   use a namespace.
	case 'setAttributeNS':
	    if (_proxy_jslib_instanceof(o, 'Element')) {
		return function (namespace, name, value) {
			   if (this!==window) o= this ;
			   return o.setAttributeNS(namespace, name.toLowerCase(),
			       _proxy_jslib_proxify_attribute(o, o.attributes[name], name, value) ) ;
		       } ;
	    } else {
		return _handle_default() ;
	...
	case 'getAttributeNS':
		...
		return function (namespace, localName) {
			   var attr_val= o.getAttributeNS(namespace, localName) ;
			   return _proxy_jslib_proxify_attribute(o, o.attributes[localName], localName, attr_val, 1) ;
		       } ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'getPropertyValue':
		...
		return function (propertyName) {
			   return _proxy_jslib_supports_proxy_class
			       ? o.getPropertyValue(propertyName)
			       : _proxy_jslib_proxify_css(o.getPropertyValue(propertyName), 1) ;
		       } ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'cssText':
	    if (_proxy_jslib_instanceof(o, 'CSSStyleDeclaration')) {
		return _proxy_jslib_supports_proxy_class  ? val  : _proxy_jslib_proxify_css(val, 1) ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'referrer':
	    ...
	    } else if (_proxy_jslib_instanceof(o, 'Request')) {
		if (o[property]=='client' || o[property]=='no-referrer')
		    return o[property] ;
		return _proxy_jslib_full_url(o[property], void 0, true) ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'toLocaleString':
		...
		return function () {
			   if (this!==window) o= this ;
			   if (_proxy_jslib_modified_prototypes) return o[property]() ;
			   return _proxy_jslib_parse_full_url(o[property]())[3] ;
		       } ;
	    } else {
		if (typeof o=='function') {
		    ...
		    return function () {
			       ...
			       // don't unproxify non-functions or native functions
			       if (!s.match(/^\s*function\b/) ||
				   s.match(/^\s*function\b\s[\$\w\.]*\([^)]*\)\s*\{\s*\[\s*native\s+code\s*\]\s*\}/))
				       return s ;
			       return _proxy_jslib_proxify_js(s, 0, 0, in_new_statement, true) ;
			   } ;
		} else {
		    return _handle_default() ;
		}
...
//		    alert('Tried to access opener window, but has different domain; domains are [' + _proxy_jslib_document_domain + '] and [' + w._proxy_jslib_document_domain + ']') ;
	...
	case 'contentDocument':
	    if (_proxy_jslib_instanceof(o, 'HTMLElement') && o.tagName.toLowerCase()=='iframe') {
		 if (!_proxy_jslib_document_domain) _proxy_jslib_init_domain(window) ;
		 if (!o.contentWindow._proxy_jslib_document_domain) _proxy_jslib_init_domain(o.contentWindow) ;
		 if (o.contentWindow._proxy_jslib_document_domain
		     && (o.contentWindow._proxy_jslib_document_domain!=_proxy_jslib_document_domain))
		 {
		     return void 0 ;
		 } else {
	...
	case 'contentWindow':
	    if (_proxy_jslib_instanceof(o, 'HTMLElement') && o.tagName.toLowerCase()=='iframe') {
		 ...
		 if (o.contentWindow._proxy_jslib_document_domain
		     && (o.contentWindow._proxy_jslib_document_domain!=_proxy_jslib_document_domain))
		 {
		     return _proxy_jslib_dup_window_safe(o.contentWindow) ;
		 } else {
	...
	// make use of browser's URL-parsing by creating a Link object and accessing o[property]
	case 'protocol':
	case 'host':
	case 'hostname':
	case 'port':
	...
	case 'search':
	    if (_proxy_jslib_instanceof(o, 'Link')) {
		var temp_link= o.ownerDocument.createElement('a') ;
		temp_link.href= _proxy_jslib_parse_full_url(o.href)[3] ;
		return temp_link[property] ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'String':
	    if (_proxy_jslib_instanceof(o, 'Window')) {
		return _proxy_jslib_String_constructor ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'replaceState':
		...
		return function(data, title, url) {
		    ...
		    if (!o_pu || (o_pu[3]!=doc_pu[3])) {
			//alert('History.'+property+'() not allowed unless origins match: ['+o_pu[3]+'] ['+doc_pu[3]+']') ;
			return void 0 ;
		    }
		    return property=='pushState'
			? _proxy_jslib_ORIGINAL_HISTORY_pushState.call(o, data, title, new_url)
			: _proxy_jslib_ORIGINAL_HISTORY_replaceState.call(o, data, title, new_url) ;
		}
	    } else {
		return _handle_default() ;
	    }
...
//alert('in execCommand(); params=['+cmd+']['+do_UI+']['+value+']') ;  //jsm-- remove
	...
	case 'assign':
	    if (_proxy_jslib_isclass(o, 'Object')) {
	...
	case 'defineProperty':
	    if (_proxy_jslib_isclass(o, 'Object') || _proxy_jslib_isclass(o, 'Reflect')) {
		return function(obj, name, desc) {
			   if ('value' in desc) {
			       desc.value= _proxy_jslib_assign('', obj, name, '=', desc.value, true) ;
			   }
			   return o.defineProperty(obj, name, desc) ;  // to handle other descriptor fields
		       } ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'defineProperties':
		...
		return function(obj, descs) {
			   ...
			   return o.defineProperties(obj, descs) ;  // to handle other descriptor fields
		       } ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'getOwnPropertyDescriptor':
		...
		return function(obj, name) {
			   var desc= o.getOwnPropertyDescriptor(obj, name) ;
			   if ('value' in desc) desc.value= _proxy_jslib_handle(obj, name, obj[name]) ;
			   return desc ;
		       } ;
	    } else {
		return _handle_default() ;
	...
	case 'get':
	    if (_proxy_jslib_isclass(o, 'Reflect')) {
		return function(target, property_key, receiver) {
			   // is this sufficient?
			   return _proxy_jslib_handle(target, property_key, target[property_key]) ;
		       } ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'set':
	    if (_proxy_jslib_isclass(o, 'Reflect')) {
		return function(target, property_key, value, receiver) {
			   try {
			       _proxy_jslib_assign('', target, property_key, '=', value) ;
			   } catch(e) {
			       return false ;
			   }
			   return true ;
	...
	case 'style':
	    // note that we test o.style here, not o
	    if (   _proxy_jslib_instanceof(o.style, 'CSS2Properties')
		|| _proxy_jslib_instanceof(o.style, 'CSSStyleDeclaration') )
	    {
		// interestingly, a Proxy object appears in every way to be of
		//   the underlying class, in this case CSSStyleDeclaration, so
		//   _proxy_jslib_instanceof() works on it.
		return _proxy_jslib_supports_proxy_class
		    ? new Proxy(o.style, _proxy_jslib_style_handler)  : o.style ;
	    } else {
		return _handle_default() ;
	    }
	...
	case 'getComputedStyle':
	    if (_proxy_jslib_instanceof(o, 'Window')) {
		return function(element, pseudo_elt) {
			   return _proxy_jslib_supports_proxy_class
			       ? new Proxy(o[property](element, pseudo_elt), _proxy_jslib_style_handler)
			       : o[property](element, pseudo_elt) ;
		       } ;
	    } else {
		return _handle_default() ;
	    }
...
function _proxy_jslib_assign (prefix, o, property, op, val, dont_set) {
    var new_val, match ;

    // protect our namespace
    if (property && (match= property.toString().match(/^_proxy(\d*)_/)))
	property= '_proxy' + (match[1]-0+1) + property.toString().replace(/^_proxy(\d*)/, '') ;
	...
	if (op=='=')
	    ...
	    eval('new_val' + op + 'val') ;
	    return o.setItem(property, new_val) ;
	}
...
//if (o==null) alert('in assign, o is null, property, caller=\n['+property+']\n['+arguments.callee.caller+']') ;   // jsm-- remove in production release?
	...
	case 'action':
	    if (_proxy_jslib_instanceof(o, 'Element')) {
		var o_tagname= o.tagName.toLowerCase() ;
		if ((o_tagname=='form' || o_tagname=='isindex') && property=='action') {
	...
	case 'documentURI':
	    ...
	    } else if ( (o.ownerDocument && (o.ownerDocument.defaultView||o.ownerDocument.parentWindow) &&
	    ...
	    } else if (property=='documentURI' && _proxy_jslib_instanceof(o, 'Document')) {
		o[property]= _proxy_jslib_full_url(new_val, o) ;
	...
	case 'srcdoc':
	    if (_proxy_jslib_instanceof(o, 'HTMLElement') && (o.tagName.toLowerCase()=='iframe')) {
		o.srcdoc= _proxy_jslib_proxify_html(val, null)[0] ;
		return val ;
	    } else {
	...
	case 'cssText':
	    if (_proxy_jslib_instanceof(o, 'CSSStyleDeclaration')) {
		o[property]= _proxy_jslib_supports_proxy_class  ? val  : _proxy_jslib_proxify_css(val) ;
		return val ;
	    } else {
		return _assign_default() ;
	    }
	...
	case 'nodeValue':
	    ...
	    if (_proxy_jslib_instanceof(o, 'Attr')) {
		o[property]= _proxy_jslib_proxify_attribute(void 0, o, o.name, new_val) ;
	    } else if (_proxy_jslib_instanceof(o, 'Node')) {
		o[property]= _proxy_jslib_proxify_attribute(o, void 0, o.name, new_val) ;
	    }
	    return new_val ;
...
//		else alert('Warning: tried to set document.domain to illegal value: ['+val+'] existing domain: ['+w._proxy_jslib_document_domain+']') ;  // jsm
	...
	case 'background':
	    if (_proxy_jslib_instanceof(o, 'HTMLElement')) {
		o[property]= _proxy_jslib_full_url(val, o) ;
		return val ;
	    } else if (_proxy_jslib_instanceof(o, 'CSS2Properties') || _proxy_jslib_instanceof(o, 'CSSStyleDeclaration'))
	    {
		o[property]= _proxy_jslib_supports_proxy_class ? val  :  _proxy_jslib_proxify_css(val) ;
	...
	case 'style':
		...
		if (typeof val=='string') {
		    ...
		    o[property]= _proxy_jslib_supports_proxy_class
			? new Proxy(val, _proxy_jslib_style_handler)  : val ;
		    return val ;
		}
	    } else {
		return _assign_default() ;
	...
	if (dont_set) {
	    ...
	    return eval('o[property]'+opmod+'val') ;
	} else {
	    if (op=='=') return o[property]= val ;   // optimization to not use eval
...
function _proxy_jslib_new(o, oclass) {
    ...
    var fnname= '' ;
    try { fnname= _proxy_jslib_ORIGINAL_FUNCTION_toString.call(o).match(/^\n?function\b\s*([\$\w\.]*)/)[1] } catch (e) {}
    if (fnname.match(/^(?:XMLHttpRequest|AnonXMLHttpRequest|EventSource|WebSocket|Worker|SharedWorker|Audio|Function|Request)$/))
	oclass= fnname ;
    if (!oclass) oclass= '' ;
    ...
    } else if (oclass=='Request') {
	if ((typeof arguments[2]=='string') || _proxy_jslib_instanceof(arguments[2], 'String'))
	    arguments[2]= _proxy_jslib_full_url(arguments[2]) ;
	// can leave init.referrer unchanged, as it (if a URL) will be handled on the server side
	// for now, don't change init.integrity
    }
...
//if (uri_ref.match(/\/[01]{6}[A-Z]\//)) alert('in full_url; uri_ref, caller=\n['+uri_ref+']\n['+arguments.callee.caller+']') ;   // jsm
    ...
    if (reverse) {
	if (uri_ref.match(/^(javascript|livescript)\:/i)) {
	    script= uri_ref.replace(/^(javascript|livescript)\:/i, '') ;
	    return 'javascript:' + _proxy_jslib_proxify_js(script, 0, 0, 0, 1) ;
	} else {
	    return _proxy_jslib_parse_full_url(uri_ref)[3] ;
	}
    }
...
//    if (/^x\-proxy\:\/\//i.test(uri_ref))  return '' ;
    ...
    if (uri_ref.match(/^\s*(vbs|vbscript)\s*:/i))  return undef ;
	...
	if (is_frame_src) {
	    return "javascript: '<html><head>"
		 ...
		 + ")[0]) ; document._proxy_jslib_has_jslib= 1</script></body></html>'" ;
	}
    ...
    absurl= _proxy_jslib_sanitize_url_path(absurl) ;
...
function _proxy_jslib_sanitize_url_path(absurl) {
    var ret= [] ;
    var m1= absurl.match(/^([\w\+\.\-]+\:\/\/[^\/\\\?\#]*[\/\\]?)([^\?\#]*)(.*)$/) ;
    if (!m1) return absurl ;
    var segs= m1[2].split(/[\/\\]/) ;
...
function _proxy_jslib_proxify_html(html, doc, reverse) {
    ...
    // start, comment, script_block, style_block, SVG block, MathML block,
    //   special_wo_html, special_with_html, CDATA block, decl_bang, decl_question, tag
    // special_wo_html and special_with_html each use an extra set of parentheses
    // note that a unique instance of RE must be created, in case of recursion
    var RE= new RegExp(/([^\<]*(?:\<(?![\w\/\!])[^\<]*)*)(?:(\<\!\-\-(?=[\s\S]*?\-\-\>)[\s\S]*?\-\-\s*\>|\<\!\-\-(?![\s\S]*?\-\-\>)[\s\S]*?\>)|(\<script\b[\s\S]*?\<\/script\b[\s\S]*?\>)|(\<style\b[\s\S]*?\<\/style\b[\s\S]*?\>)|(\<svg\b[\s\S]*?\<\/svg\b[\s\S]*?\>)|(\<math\b[\s\S]*?\<\/math\b[\s\S]*?\>)|(\<(textarea|xmp|title)\b[\s\S]*?\<\/\8\b[\s\S]*?\>)|(\<(select|noscript|noembed|noframes|iframe)\b[\s\S]*?\<\/\10\b[\s\S]*?\>)|(\<\!\[CDATA\[[\s\S]*?\]\]\>)|(\<\![^\>]*\>)|(\<\?[^\>]*\>)|(\<[^\>]*\>))?/gi) ;
	...
	if (match[2]) {
	    ...
	    m2= match[5].match(/^(\<\s*svg\b[^\>]*\>)([\s\S]*)(\<\s*\/svg\b[^\>]*\>)$/i) ;
	    m2[1]= _proxy_jslib_proxify_element(m2[1], doc, reverse) ;
	    out.push(m2[1], m2[3]) ;
	} else if (match[6]) {
	    m2= match[6].match(/^(\<\s*math\b[^\>]*\>)([\s\S]*)(\<\s*\/math\b[^\>]*\>)$/i) ;
	    m2[1]= _proxy_jslib_proxify_element(m2[1], doc, reverse) ;
	    out.push(m2[1], m2[3]) ;
	} else if (match[7]) {
	    m2= match[7].match(/^(\<\s*(\w+)\b[^\>]*\>)([\s\S]*)(\<\s*\/\2\b[^\>]*\>)$/i) ;
	    m2[1]= _proxy_jslib_proxify_element(m2[1], doc, reverse) ;
	    out.push(m2[1], m2[3], m2[4]) ;
	} else if (match[9]) {
	    m2= match[9].match(/^(\<\s*(\w+)\b[^\>]*\>)([\s\S]*)(\<\s*\/\2\b[^\>]*\>)$/i) ;
	    m2[1]= _proxy_jslib_proxify_element(m2[1], doc, reverse) ;
	    m2[3]= _proxy_jslib_proxify_html(m2[3], doc, reverse) ;
	    out.push(m2[1], m2[3], m2[4]) ;
	} else if (match[11]) {
	    out.push(match[11]) ;
	} else if (match[12]) {
	    out.push(_proxy_jslib_proxify_decl_bang(match[12], doc, reverse)) ;
	} else if (match[13]) {
	    out.push(_proxy_jslib_proxify_decl_question(match[13], doc, reverse)) ;

	} else if (match[14]) {
	    m2= match[14].match(/^\<\s*(\/?[A-Za-z][\w\.\:\-]*)/) ;
	    if (!m2) continue ;    // hack until we parse more rigorously
	    tag_name= m2[1].toLowerCase() ;

	    // these would indicate incomplete blocks
	    if ((tag_name=='script') || (tag_name=='style')) {
		remainder= match[14]+html.slice(last_lastIndex) ;
		break ;
	    }
	    ...
	    var new_element= _proxy_jslib_proxify_element(match[14], doc, reverse) ;
	    while (new_element==void 0 && last_lastIndex!=html.length) {
		m2= RE2.exec(html) ;
		last_lastIndex= RE.lastIndex= RE2.lastIndex ;
		match[14]+= m2[0] ;
		new_element= _proxy_jslib_proxify_element(match[14], doc, reverse) ;
	    }
...
function _proxy_jslib_proxify_script_block(script_block, doc, reverse) {
    ...
    if (attr.language && attr.language.match(/vbs/i)) return void 0 ;
    if (attr.type!=null) attr.type= attr.type.toLowerCase() ;
    if (attr.type && !(attr.type.match(/^(application\/x\-javascript|application\/x\-ecmascript|application\/javascript|application\/ecmascript|text\/javascript|text\/ecmascript|text\/livescript|text\/jscript)/)))
	return void 0 ;
	...
	attr.type= attr.language.match(/javascript|ecmascript|livescript|jscript/i)
		 ...
		 : attr.language.match(/vbs/i)       ? 'application/x-vbscript'
...
function _proxy_jslib_proxify_element(element, doc, reverse) {
    ...
    if (attr.type!=null) attr.type= attr.type.toLowerCase() ;
    if (!attr.type && attr.language) {
	type= attr.language.match(/javascript|ecmascript|livescript|jscript/i)
						? 'application/x-javascript'
	    : attr.language.match(/css/i)       ? 'text/css'
	    : attr.language.match(/vbscript/i)  ? 'application/x-vbscript'
	    : attr.language.match(/vbs/i)       ? 'application/x-vbscript'
    ...
    if (!type) type= attr.type || _proxy_jslib_default_script_type ;
...
//    for (name in attr) {
	...
	if (name.match(/^on/)) {
	    rebuild= 1 ;
	    if (attr[name].match(/^\s*(vbs|vbscript)\s*:/i)) { delete attr[name] ; continue }
	    attr[name]= _proxy_jslib_proxify_block(attr[name], type, _proxy_jslib_ALLOW_UNPROXIFIED_SCRIPTS, reverse) ;
	}
    }
...
//	}
	...
	if (   (   (doc.defaultView||doc.parentWindow)
		&& (doc.defaultView||doc.parentWindow)._proxy_jslib_base_unframes
		&& attr.target==void 0)
	...
	if (tag_name=='frame' || tag_name=='iframe') {
		...
		var old_url_start= _proxy_jslib_url_start ;
		var flags_6= _proxy_jslib_flags[6] ;
		_proxy_jslib_flags[6]= (type!==void 0)  ? type  : _proxy_jslib_default_script_type ;
		try {
		    _proxy_jslib_url_start= _proxy_jslib_url_start_by_flags(_proxy_jslib_flags) ;
...
function _proxy_jslib_proxify_attribute(element, attr, name, value, reverse) {
	...
	// don't convert href if it's not one of these four elements, or empty... hacky....
	...
	// use null for doc argument-- hack to force JS insertion
	if (element_name=='iframe')  return _proxy_jslib_proxify_html(value, null, reverse)[0] ;
	...
	if (element) {
	    ...
	    if (!type && language) {
		type= language.match(/javascript|ecmascript|livescript|jscript/i)
						    ? 'application/x-javascript'
		    : language.match(/css/i)       ? 'text/css'
		    : language.match(/vbscript/i)  ? 'application/x-vbscript'
		    : language.match(/vbs/i)       ? 'application/x-vbscript'
		    : language.match(/perl/i)      ? 'application/x-perlscript'
		    : language.match(/tcl/i)       ? 'text/tcl'
		    : '' ;
	    }
	}
	if (!type) type= _proxy_jslib_default_script_type ;
	if (type.match(/vbs/) || (language && language.match(/vbs/i))) return void 0 ;
	return _proxy_jslib_proxify_block(value, type,
			_proxy_jslib_ALLOW_UNPROXIFIED_SCRIPTS, reverse) ;
    } else if (name=='language') {
	return (value.match(/vbs/i))  ? void 0  : value ;
    ...
    } else if ((name=='data') && (element_name=='object')) {
	return _proxy_jslib_full_url_by_frame(value, null, true, reverse) ;
    } else {
	return value ;
    }
...
function _proxy_jslib_proxify_js_tokens(jsin, start, end, top_level, with_level, in_new_statement, reverse)
{
  ...
  OUTER:
	...
	} else if (token=='function' || (term_so_far=='' && (token=='get' || token=='set'))) {
	    out.push(term_so_far, token) ;
	    term_so_far= '' ;
	...
	//      } else if (RE.SET_TRAPPED_PROPERTIES[token]) {
	} else if (/^(open|write|writeln|load|eval|setInterval|setTimeout|toString|String|src|srcdoc|srcset|currentSrc|href|background|lowsrc|action|formAction|location|poster|data|URL|url|newURL|oldURL|documentURI|referrer|baseURI|useMap|longDesc|cite|codeBase|profile|cssText|insertRule|setStringValue|setProperty|backgroundImage|content|cursor|listStyleImage|host|hostname|pathname|port|protocol|search|setNamedItem|setNamedItemNS|innerHTML|outerHTML|outerText|body|parentNode|insertAdjacentHTML|setAttribute|setAttributeNS|setAttributeNode|setAttributeNodeNS|getAttribute|getAttributeNS|nodeValue|value|cookie|domain|frames|parent|top|opener|execScript|execCommand|navigate|navigator|showModalDialog|showModelessDialog|addImport|LoadMovie|close|getElementById|getElementsByTagName|appendChild|replaceChild|insertBefore|removeChild|createElement|text|textContent|origin|postMessage|pushState|replaceState|localStorage|sessionStorage|querySelector|querySelectorAll|send|setRequestHeader|withCredentials|responseURL|getPropertyValue|assign|contentDocument|contentWindow|defineProperties|defineProperty|getOwnPropertyDescriptor|get|set|style|getComputedStyle|fetch)$/.test(token)) {
	    ...
	    if (/^[\{\,]/.test(last_token) && (token=='get' || token=='set' || next_token==':')) {
		out.push(term_so_far, token) ;
		term_so_far= '' ;
		if (next_token==':') {
		    for (i= i_jsin ; i<=i_next_token ; i++) out.push(jsin[i]) ;
	...
	// distinguishing between an object literal and a block is messy
		...
		i_jsin= i_next_token+1 ;
		while (i_jsin<end && jsin[i_jsin].skip) i_jsin++ ;
		if ((jsin[i_jsin]==':') || (next_token=='get' || next_token=='set')) {
		    if (jsin[i_jsin]==':') {
			...
			if (!(jsin[i_jsin].match(RE.IDENTIFIER) || jsin[i_jsin].match(RE.STRINGLITERAL) || jsin[i_jsin].match(RE.NUMERICLITERAL)))
			    break OUTER ;
			if (jsin[i_jsin]!='get' && jsin[i_jsin]!='set') {
...
function _proxy_jslib_get_next_js_expr(jsin, start, end, allow_multiple, is_new) {
	...
	// needed since JavaScript doesn't turn String objects into strings in switch statements....  :P
	switch(typeof element=='string'  ? element  : element.valueOf()) {
...
function _proxy_jslib_tokenize_js(s) {
    ...
    var RE_IdentifierStart= _proxy_jslib_RE.IDENTIFIERSTART ;
	...
	} else if (token.match(_proxy_jslib_RE.IDENTIFIERSTART)) {
	    token= token.replace(/\\u([0-9A-Fa-f]{4})/g,
				 function (u) { return '\\u'+String.fromCharCode(parseInt(u,16)) } ) ;
...
function _proxy_jslib_set_RE() {
	...
	RE.IDENTIFIERSTART= new RegExp('^(?:'+RE.IdentifierStart+')') ;
...
// As of version 2.2.2, this has been rewritten to use _proxy_jslib_isa(), which
//   checks win[classname].prototype.isPrototypeOf(o) in each Window object.
//   The old duck-typing code is now obsolete, but we'll leave it here until we're
//   sure the new version works.
function _proxy_jslib_instanceof(o, classname) {
    if ((o==null) || ((typeof(o)!='object') && (typeof(o)!='function')))
	return false ;
    if (classname=='Location' && ("_proxy_jslib_original_win" in o))  return true ;
    if (classname=='Link')
	return _proxy_jslib_isa(o, 'HTMLAnchorElement') || _proxy_jslib_isa(o, 'HTMLAreaElement') ;

    try {
	return _proxy_jslib_isa(o, classname) ;
    } catch(e) {
	// usually gets here when window[classname] doesn't exist, like for deprecated classes
	// examples include Layer, CSS2Properties, AnonXMLHttpRequest
	return false ;
    }
	...
	switch(classname) {
	    ...
	    case 'Location':
		// not entirely comfortable using _proxy_jslib_original_win here, but at least
		//   one library uses pseudo-Location objects, as CGIProxy does, so we need to
		//   avoid false positives with those.  Wish instanceof worked everywhere!  :(
		return ("reload" in o) && ("protocol" in o)  && ("search" in o) && ("_proxy_jslib_original_win" in o) ;
	    case 'Link':
...
// similar to _proxy_jslib_instanceof(), but instead walks recursively through
//   the given window and all subframes of it to see if o.constructor matches
//   the class in any of the Window objects.
// Note that Object.isPrototypeOf(o) looks at the whole prototype chain for o.
// This is a major performance bottleneck.  :(  It's much slower than the
//   previous duck-typing of _proxy_jslib_instanceof().  Because of this, we
//   try to optimize it.
function _proxy_jslib_isa(o, classname) {
    // check current window first, since o will usually be in that.  If first
    //   check below fails but o is still an Object in current window, then
    //   it's not one of classname.
    if (!window[classname]) return false ;
    if (window[classname].prototype.isPrototypeOf(o)) return true ;
    if (Object.prototype.isPrototypeOf(o)) return false ;
    return _proxy_jslib_isa_in_window_tree(o, classname, top) ;
}

function _proxy_jslib_isa_in_window_tree(o, classname, win) {
    if (win[classname].prototype.isPrototypeOf(o)) return true ;
    if (win.Object.prototype.isPrototypeOf(o)) return false ;
    var f= win.frames ;
    for (var i= 0 ; i<f.length ; i++)
	if (_proxy_jslib_isa_in_window_tree(o, classname, f[i])) return true ;
    return false ;
}

// like above, but o is expected to be a class (constructor), e.g. Object.
function _proxy_jslib_isclass(o, classname, win) {
    win= win || top ;
    if (classname=='String' && o===win._proxy_jslib_String_constructor) return true ;
    else if (o===win[classname]) return true ;
    var f= win.frames ;
    for (var i= 0 ; i<f.length ; i++)
	if (_proxy_jslib_isclass(o, classname, f[i])) return true ;
    return false ;
}


function _proxy_jslib_is_native_or_host_class(o) {
    if (_proxy_jslib_isclass(o, 'String')) return true ;   // the only constructor we override
    if (typeof(o)=='function') {
	var funcst= _proxy_jslib_ORIGINAL_FUNCTION_toString.call(o) ;
	return /^\s*function\b\s[\$\w\.]*\([^)]*\)\s*\{\s*\[\s*native\s+code\s*\]\s*\}/.test(funcst) ;
    } else {
	return false ;    // is this sufficient?	
    }
}
...
function _proxy_jslib_parse_url(in_URL) {
    ...
    if (u= in_URL.match(/^about:([^#]*)(.*)/i))
	return [ in_URL, 'about:', '', '', '', '', u[1], '', u[2] ] ;
...
//	return [ 'about:blank', 'about:', '', '', '', '', 'blank', '', '' ] ;
    ...
    u= in_URL.match(/^([\w\+\.\-]+\:)?\/\/([^\/\?\#\@]*\@)?((\[[^\]]*\]|[^\:\/\?\#]*)(\:[^\/\?\#]*)?)([^\?\#]*)([^#]*)(.*)$/) ;
    if (u==null) return [in_URL, '', '', '', '', '', '', '', '']  ;   // if pattern doesn't match
...
function _proxy_jslib_parse_full_url(in_URL) {
    ...
    if (in_URL.match(/^about:/i)) return ['', '', '', in_URL] ;
...
EOF
	...
	my $bodyref= $content_encoding_header ? \$JSLIB_BODY_GZ : \$JSLIB_BODY ;
	print $STDOUT "$HTTP_1_X 200 OK\015\012",
		      "${session_cookies}Date: $date_header\015\012",
		      "Cache-Control: $cache_control\015\012",
		      "Content-Type: application/x-javascript\015\012",
		      "Content-Length: ", length($$bodyref), "\015\012",
...
# $run_method is from config_wizard(), or undef.
# If $upgrade, try to upgrade all required modules to their latest versions,
sub install_modules {
    my($run_method, $upgrade)= @_ ;   # $run_method is from config_wizard(), or undef
    my(@modules, $installer, $installed_local_lib, %env) ;
    ...
    push(@modules, qw(FCGI  FCGI::ProcManager))
	if $run_method eq 'f' or $run_method eq '' ;


    # Use a package manager if available, else use cpan
    if ($^O eq 'MSWin32') {
	$installer= 'cpan' ;
    } else {
	if ($>==0) {
	    # Try to use a package manager if available, else use cpan .
	    # Note that cpan may not behave well with package managers, so don't
	    #   use it when running as root if a package manager is available--
	    #   instead, let the user use cpan at their discretion.
	    $installer= `which dnf 2>/dev/null`     ? 'dnf'        # Fedora
		      : `which yum 2>/dev/null`     ? 'yum'        # CentOS, old Fedora
		      : `which apt-get 2>/dev/null` ? 'apt-get'    # Debian, Ubuntu, etc.
		      :                               'cpan' ;
	} else {
	    # cpan is more safe for non-root users than for root users, since
	    #   they're not modifying system packages.
	    $installer= 'cpan' ;
	}
    }
	...
	foreach (@modules) {
	    eval "require $_" ;
	    push(@modules_to_install, $_)  if $@ ;
	}
	if (!@modules_to_install) {
	    print "All required CPAN modules are already installed.\n\n" ;
	    return ;
	}
    }


    # yum and dnf have a "perl(...)" syntax for installing CPAN modules, to
    #   accommodate using the CPAN module names rather than the package names
    #   (which vary by OS).
    if ($installer eq 'yum' or $installer eq 'dnf') {
	my @yum_modules= map { "perl($_)" } @modules_to_install ;
	print "Trying $installer to install [@modules_to_install]...\n\n" ;
	system($installer, 'install', @yum_modules) ;


    # Install one package at a time with apt-get, due to package name complications.
    } elsif ($installer eq 'apt-get') {
	foreach my $module (@modules_to_install) {
	    # First, guess at the package name:  Foo::Bar -> libfoo-bar-perl
	    my $pkg_name= 'lib' . join('-', (map {lc} split(/::/, $module)), 'perl') ;
	    print "Trying $installer to install $module...\n\n" ;
	    system('apt-get', 'install', $pkg_name) ;   # upgrades if newer package available

	    # If that doesn't work, try first package name returned by "apt-cache search".
	    eval "require $module" ;
	    if ($@) {
		($pkg_name)= (`apt-cache search '$module'`)[0]=~ /^(\S+)/ ;
		if ($pkg_name=~ /^lib[\w-]+-perl$/) {
		    system('apt-get', 'install', $pkg_name) ;
		}
	    }
	}


    } elsif ($installer eq 'cpan') {
	# This tells CPAN to use defaults instead of asking the user for everything.
	$ENV{PERL_MM_USE_DEFAULT}= 1 ;

	# When using cpan to install modules, umask needs to be 022.  Otherwise,
	#   ~/.cpan/build could be created with permissions that are either
	#   world-writable or group-writable, which prevents File::Temp from
	#   using that directory when installing local::lib due to a security
	#   feature in File::Temp .
	my $old_umask= umask(022) ;

	# IO::Compress::Lzma sometimes has trouble installing because it requires
	#   IO::Compress::Base to be of the same or higher version.  IO::Compress::Base
	#   is a standard module, but upgrade it if we're installing IO::Compress::Lzma
	#   to avoid problems.  Install IO::Compress::Base before IO::Compress::Lzma .
	# Only do this if using cpan, not package managers, because any package with
	#   IO::Compress::Base will typically be out of date and thus not good enough
	#   for IO::Compress::Lzma.  :(
	unshift(@modules_to_install, 'IO::Compress::Base')
	    if grep { $_ eq 'IO::Compress::Lzma' } @modules_to_install ;
	...
	if ($>!=0) {
		...
		if ($dist= CPAN::Shell->expand('Module', 'local::lib')->distribution) {
		    my $lld_safe= $LOCAL_LIB_DIR=~ s/'//g ;
		    $dist->{prefs}{pl}{commandline}= $LOCAL_LIB_DIR ne ''
			? "$^X Makefile.PL '--bootstrap=$lld_safe'"
			: "$^X Makefile.PL --bootstrap" ;
		    require lib ;
		    lib->import(CPAN::FirstTime::_local_lib_inc_path()) ;
		    print "Trying $installer to install local::lib...\n\n" ;
		    eval { $dist->install } ;
		}
		if (!$dist or $@) {
		    die "Can't install local::lib: $@\n" ;
		} else {
		    # local::lib->import() has a bug where the first import()
		    #   actually removes some paths from @INC, and the second
		    #   import() adds the paths correctly.  Thus, call it twice.
		    require local::lib ;
		    local::lib->import($LOCAL_LIB_DIR) ;
		    local::lib->import($LOCAL_LIB_DIR) ;
	    ...
	    #   user's $HOME.
	    @ENV{qw(PERL_LOCAL_LIB_ROOT PERL_MB_OPT PERL_MM_OPT PERL5LIB)}= ('') x 4 ;
	}


	# Try to install the modules.
	# CPAN::Shell->install() doesn't always retore the current directory, so
	#   do it explicitly.
	foreach my $module (@modules_to_install) {
	    my $oldcwd= getcwd() ;
	    print "Trying $installer to install $module...\n\n" ;
    ...
    my $failed_modules= join("\n  ", @failed) ;

    # For various reasons, IO::Compress::Lzma often has trouble installing, but
    #   it's usually not needed.
    my $lzma_message= (grep { $_ eq 'IO::Compress::Lzma' } @failed)  ? <<EOM  : '' ;
========================================================================
Don't worry too much about IO::Compress::Lzma failing to install-- CGIProxy
only uses it for an uncommon kind of Flash file, so everything other than
those will still work.  If you want to install it manually, try
"cpan IO::Compress::Lzma" from the command line.

Note that IO::Compress::Lzma requires that IO::Compress::Base has the same
or higher version number.  You can update IO::Compress::Base to the latest
version with "cpan IO::Compress::Base".
EOM
...
You need to install them yourself, using the "cpan" utility or your system's
package manager.
...
EOF
	...
	if ($^O eq 'MSWin32') {
	    # This isn't currently called, since we don't use local::lib on Windows.
	    print <<EOI ;
...
install under the system directory.  To let Perl programs find modules under
your own directory, please add these environment variables in your
Control Panel's System applet:

$env
EOI
	} else {
	    # Guess at the shell startup file.  This is not perfect!
	    my $startup_file= $ENV{SHELL}=~ /tcsh$/  ? '.tcshrc'
			    : $ENV{SHELL}=~ /csh$/   ? '.cshrc'
			    : $ENV{SHELL}=~ /ksh$/   ? '.profile'
			    : $ENV{SHELL}=~ /bash$/  ? '.bashrc'
			    : $ENV{SHELL}=~ /zsh$/   ? '.zshrc'
			    :                          '.profile' ;
	    print <<EOI ;
...
*****************************************************************************
We had to install the module local::lib in order to install Perl modules
under your own directory rather than requiring root permissions to install
under the system directory.  To let Perl programs find modules under your
own directory, these lines need to be added to your shell startup file
"$startup_file", in your home directory:

$env
EOI
	    my $resp= ask_user('', 'Do you want them to be added for you right now? [y/n]', qr/^[yn]/i) ;
	    if ($resp=~ /^y/i) {
...
# Added after installing the local::lib CPAN module.
$env
EOE
		close(STARTUP) ;
	    }
	...
	print <<EOI  unless $LOCAL_LIB_DIR eq $local_lib_dir ;
*****************************************************************************
...
EOI
    ...
    die "ERROR: CGIProxy currently requires the JSON module for security in JavaScript,\nand it wasn't installed successfully.\n" if $@ ;
}
...
# Returns 1 if database already exists, 2 upon successful database creation,
...
sub create_database_as_needed {
	...
	my $old_umask= umask(0) ;
	my($data_dir_mode, $database_mode)=
	      ($>==0)                      ? (0700, 0600)
	    : ($>==$RUN_AS_USER_ID)        ? (0700, 0600)
	    : $)=~ /\b$RUN_AS_GROUP_ID\b/  ? (0770, 0660)   
	    :                                (0777, 0666) ;
	my $data_dir= File::Spec->catfile($PROXY_DIR, 'sqlite') ;
	-d $data_dir or mkdir($data_dir, $data_dir_mode) or die "can't mkdir [$data_dir]: $!" ;
	$DBH||= DBI->connect("dbi:SQLite:dbname=$DB_FULLPATH", '', '', { AutoCommit => 1, sqlite_unicode => 1 }) ;
	&HTMLdie("Can't connect to SQLite database: $DBI::errstr")  unless $DBH ;
	if ($^O ne 'MSWin32') {
	    if ($>==0) {
		chown($RUN_AS_USER_ID, $RUN_AS_GROUP_ID, $data_dir, $DB_FULLPATH)
		    or die "can't chown $RUN_AS_USER:$RUN_AS_GROUP $data_dir $DB_FULLPATH: $!" ;
	    } elsif ($)=~ /\b$RUN_AS_GROUP_ID\b/) {
		chown(-1, $RUN_AS_GROUP_ID, $data_dir, $DB_FULLPATH)
		    or die "can't chown :$RUN_AS_GROUP $data_dir $DB_FULLPATH: $!" ;
	    }
	}
	chmod($database_mode, $DB_FULLPATH)  or &HTMLdie(["Can't chmod %s: %s", $DB_FULLPATH, $!]) ;
	umask($old_umask) ;
    ...
    return 1 if $DBH->tables(undef, undef, 'session', 'TABLE') ;
...
sub connect_to_db {
	...
	if ($DB_DRIVER eq 'SQLite') {
	    $DBH= DBI->connect("dbi:SQLite:dbname=$DB_FULLPATH", '', '',
...
# Read all config variables from config file.
sub read_config_file {
    my($config_file)= @_ ;

    # This routine is not called if $HAS_INITED, so this is efficient enough.
    my(%is_config_var) ;
    @is_config_var{@CONFIG_VARS}= (1) x @CONFIG_VARS ;

    # Check validity of every variable name in config file.
    # jsm-- this needs to handle when two statements are on one line!
	...
	my($varname)= split(/=/) ;
	$varname=~ s/^\s*|\s*$//g ;
	&HTMLdie("Unsupported configuration variable in $config_file : $varname\n")
	    unless $is_config_var{$varname} ;
    }
    close($config) ;

    # Now we know all config variables are valid.

    # Run config file with do .
    # Currently ignores return value; could change this if we appended "1 ;" to config file.
    do $config_file ;
    &HTMLdie("Can't parse $config_file : $@")  if $@ ;
}


sub write_config_file {
    my($config_file, $ownerid, $groupid, $config_mode)= @_ ;
    ...
    my $tmp_fh= File::Temp->new() or die "Can't open temporary file for writing: $!\n" ;
    my $tmp_fname= $tmp_fh->filename ;

    print $tmp_fh <<EOH ;
#-----------------------------------------------------------------------
# CGIProxy $PROXY_VERSION configuration file
#-----------------------------------------------------------------------
# This is a configuration file for CGIProxy $PROXY_VERSION .  It was
#   created by CGIProxy during the initialization process.  For a short
#   description of each configuration variable in here, see
#   https://www.jmarshall.com/tools/cgiproxy/options.html
# For full information about each configuration variable, see the
#   configuration section of nph-proxy.cgi , near the top of the file.
#   Most configuration variables are fully explained there.
# You can easily change this configuration by running "$ZERO config",
#   which presents a menu-based tool to edit each variable in here.  You
#   can also change the configuration by editing this file directly, but
#   make sure that whatever you enter here are valid literal values in Perl.
#   Note that every line should end with ";" .
# Lines that begin with "#", like these instructions, are called
#   "comments" and are safely ignored.  Blank lines are also ignored.
# Configuration variables that start with "\$" are called "scalars"
#   and should be set to a single number or string (a string is a
#   quoted sequence of characters).
# Configuration variables that start with "\@" are called "arrays"
#   and should be set to a list of zero or more values, separated by
#   "," and with the whole list enclosed in "(" and ")".
# Configuration variables that start with "%" are called "hashes" and
#   can be set the same way as lists.  "=>" acts like a "," for hashes.
# NOTE:  PLEASE PUT EACH ARRAY OR HASH ON A SINGLE LINE IN THIS CONFIGURATION
#   FILE; THIS ALLOWS CERTAIN ERROR-CHECKING BY CGIPROXY.  This
#   requirement may change in the future.
# For more information on Perl variables, please see the Perl
#   documentation available online, or run "perldoc perlintro" from
#   the command line.
EOH

    for my $catname (@CONFIG_CATEGORIES) {
	print $tmp_fh "\n#---- $catname configuration ----------------------------\n" ;
	print $tmp_fh "$_= ", dump_var_by_name($_), " ;\n"
	    foreach @{$CONFIG_CATEGORIES{$catname}} ;
    }

    close($tmp_fh) ;

    chmod($config_mode, $tmp_fname) or die "can't chmod $tmp_fname $!" ;

    # Using mv() instead of move() will keep permissions, but regrettably not owner
    #   and group.  Thus, we have a small race condition.  :P  I think it will only
    #   result in CGIProxy not running for a split second, not in a security hole.
    require File::Copy ;
    File::Copy::mv($tmp_fname, $config_file) or die "Can't move config file $tmp_fname into place $config_file : $!\n" ;
    chown($ownerid, $groupid, $config_file) or die "can't chown $ownerid:$groupid $config_file : $!"
	if $>==0 ;

    print "\nNew configuration file written to $config_file .\n\n" ;
}



# Data::Dumper only dumps scalars-- for lists and hashes, it dumps a reference.
#   This routine dumps a global by full name, including sigil, and dumps the full
#   list (surrounded by parentheses) for either a list or a hash.
sub dump_var_by_name {
    my ($varname)= @_ ;
    $varname=~ /^[\$\@\%][A-Za-z]\w+$/  or die "Bad variable name \"$varname\".\n" ;
    ...
    my($sigil, $vname)= (substr($varname, 0, 1), substr($varname, 1)) ;
    no strict 'refs' ;
    if ($sigil eq '$') {
	return scalar Data::Dumper->Dump([$$vname]) ;
    } elsif ($sigil eq '@') {
	my $dump= Data::Dumper->Dump([\@$vname]) ;
	(substr($dump, 0, 1), substr($dump, -1))= ('(', ')') ;
	return $dump ;
    } elsif ($sigil eq '%') {
	my $dump= Data::Dumper->Dump([\%$vname]) ;
	(substr($dump, 0, 1), substr($dump, -1))= ('(', ')') ;
...
# Run an OS-specific and run-method-specific configuration wizard.
# Note that config variables must be set before running this.
# Returns ($run_method, $server_type, $server_conf_full, $server_root) .
sub config_wizard {
    my($run_method, $server_header, $server_type, $server_conf_file,
       $server_conf_full, $server_root) ;


   print <<EOI ;

For best results, please run this while your Web server is running.  This
makes it easier to auto-detect certain settings.
EOI

    my $proxy_dir_q= $^O eq 'MSWin32'  ? <<EOQ1  : <<EOQ2 ;
CGIProxy needs its own directory where it can store its configuration file
and other things.  Set it to an absolute pathname, i.e. starting with "\\"
and possibly a leading drive letter, e.g. "C:\\Users\\...".  It can't
include ".." .  If the directory doesn't exist, it will be created.
EOQ1
CGIProxy needs its own directory where it can store its configuration file
and other things.  Set it to an absolute pathname, i.e. starting with "/".
It can't include ".." .  If the directory doesn't exist, it will be created.
EOQ2
    $PROXY_DIR= ask_user($proxy_dir_q,
			 'Which directory should $PROXY_DIR be set to?',
			 sub {
			     return 0 if $_[0]=~ /\.\./ ;
			     return ($^O eq 'MSWin32')  ? $_[0]=~ m#^(?:[A-Za-z]:)?(?:/|\\)#
							: substr($_[0], 0, 1) eq '/' ;
			 },
			 $PROXY_DIR) ;
    print "\n" ;
   ...
   $server_header= get_server_header(443) ;
   $server_type= $server_header=~ /\bApache\b/i    ? 'Apache'
	       : $server_header=~ /\bnginx\b/i     ? 'nginx'
	       : $server_header=~ /\blighttpd\b/i  ? 'lighttpd'
	       :                                     'other' ;   # including no Server: header
   print "\nWe think you're using the \"$server_type\" Web server.\n\n" ;
   $run_method= ask_user(<<EOQ, 'Which way do you want to run CGIProxy? [1-9]', qr/^[1-9]/i) ;
...
EOQ
    ($run_method, $server_type)= @{([qw(c Apache)],
				    [qw(c lighttpd)],
				    [qw(c other)],
				    [qw(m Apache)],
				    [qw(f Apache)],
				    ...
				    [qw(f lighttpd)],
				    [qw(f other)],
				    [qw(e embedded)])[$run_method-1]} ;


    # Unless running the embedded server, read server config into $server_conf_full .
    ($server_conf_full, $server_root)= get_server_config_unix($server_type)
	unless $run_method eq 'e' or $^O eq 'MSWin32' ;

    print "\n" ;

    # Generate a random $SECRET_PATH if needed-- now used for all run methods.
    if ($SECRET_PATH eq '') {
	$SECRET_PATH= random_string(12) ;
	print "Setting \$SECRET_PATH to \"$SECRET_PATH\".\n\n" ;
    }


    # Set other required config variables depending on $run_method .

    # Only call this on Unix, and only if these are not already set.
    if ($^O ne 'MSWin32' and ($RUN_AS_USER eq '' or $RUN_AS_GROUP eq '')) {
	($RUN_AS_USER, $RUN_AS_GROUP)=
	    get_run_as_user_group($run_method, $server_type, $server_conf_full) ;
	fix_run_as_user_group() ;   # needed since $RUN_AS_USER_ID etc. are used during installation

	print "Setting \$RUN_AS_USER=\"$RUN_AS_USER\" and \$RUN_AS_GROUP=\"$RUN_AS_GROUP\".\n\n" ;
    }

    # Set $LOCAL_LIB_DIR to an absolute path before writing out the config
    #   file, in case ownership of the script file changes.
    fix_local_lib_dir()  if $LOCAL_LIB_DIR ;


#	... ;   # install certificates, etc.-- everything for an embedded server


    # Done with per-OS, per-install-method configuration wizard.



    # Make sure outgoing connections work.
    if (!$HTTP_PROXY) {
	if (newsocketto_simple('S_TEST', 'jmarshall.com', 443)) {
	    no warnings 'once' ;
	    close(S_TEST) ;
	} else {
	    print <<EOW ;
*** WARNING:  can't make a test connection to the CGIProxy website at
jmarshall.com .  This isn't a problem for installation, but you may need to set
\$HTTP_PROXY and \$SSL_PROXY in the "server/network environment" section later,
when you see the configuration menu.  You may also need to set other options
in the same section.

This could also mean that your server doesn't allow outgoing connections.
...
Now, you can individually modify any configuration settings using the menus
below.  From this point forward, everything is optional-- CGIProxy should
work with the current settings if you exit and save now.  If you want to
modify any configuration settings later without running the wizard again,
run "./nph-proxy.cgi config".
EOT


    # Now, let users tweak any settings.
    config_menu() ;


    return ($run_method, $server_type, $server_conf_full, $server_root) ;
}



sub get_server_config_unix {
    my($server_type)= @_ ;
    my($server_conf_file, $server_conf_full, $server_root) ;

    if ($server_type eq 'Apache') {
	my($httpd, $httpd_from_ps, $httpd_args) ;

	# See if httpd or apache process is running.
	my $ps= (`ps -eo args|grep '[h]ttpd'`  or  `ps -eo args|grep '[a]pache'`)[0] ;
	if ($ps) {
	    ($httpd_from_ps, $httpd_args)= $ps=~ /^\S+(.*)/ ;
	    # These are two command-line arguments to the Apache executable.
	    # jsm-- consider supporting -C ServerRoot
	    ($server_root)= $httpd_args=~ /[ \t]-d[ \t]*(\S+)/ ;
	    ($server_conf_file)= $httpd_args=~ /[ \t]-f[ \t]*(\S+)/ ;
	}

	# Otherwise, search the path for an appropriate executable.
	chomp($httpd= (`which apache2ctl` or `which httpd2` or `which apache2` or
		       `which apachectl` or `which httpd` or `which apache`) ) ;
	$httpd||= $httpd_from_ps ;

	return unless $httpd ;    # (undef, undef)
	#die "ERROR: Can't find the Apache executable file.\n"  unless $httpd ;
	die "ERROR: Invalid Apache executable \"$httpd\".\n" if $httpd=~ /['\\]/ ;

	# Extract relevant default settings.
	my $httpd_V= `'$httpd' -V` ;
	($server_root)= $httpd_V=~ m#\bHTTPD_ROOT="(/.+?)"#  unless $server_root ne '' ;
	return  unless $server_root ne '' ;
	#die "ERROR: Can't find HTTPD_ROOT for Apache.\n"  unless $server_root ne '' ;
	($server_conf_file)= $httpd_V=~ m#\bSERVER_CONFIG_FILE="(.+?)"#  unless $server_conf_file ne '' ;
	return (undef, $server_root)  unless $server_conf_file ne '' ;
	#die "ERROR: Can't find Apache config file name.\n"  unless $server_conf_file ne '' ;
	$server_conf_file= File::Spec->catdir($server_root, $server_conf_file)  unless $server_conf_file=~ m#^/# ;

	# As a simple protection, make sure $server_conf_file is owned by root
	#   and not writable by anyone but root.
	my $mode= (stat($server_conf_file))[2] ;  # contains file type too, but & below works
	if ((stat(_))[4] or ($mode & 022)) {
	    unsafe_conf_file_warning($server_conf_file) ;

	# httpd.conf is not always readable by non-root.
	} elsif (-r $server_conf_file) {
	    # Try to get Apache environment variables from envvars.  If not,
	    #   then initialize a hash cloned from the current environment.
	    my $apache_env= get_apache_env() || { %ENV } ;

	    # Note that this returns all directives with their parameters quoted.
	    $server_conf_full= read_and_expand_httpd_conf($server_conf_file, $apache_env, $server_root) ;

	    # If ServerRoot is defined in httpd.conf, then use it henceforth.
	    # Note that this means it's potentially a different server root
	    #   from what was used to determine the location of httpd.conf ,
	    #   which is a little weird....
	    $server_root= $1  if $server_conf_full=~ m#^\s*ServerRoot\s+['"]?(/[^'"]*)#mi ;
	}


    } elsif ($server_type eq 'nginx') {
	my($nginx, $nginx_args) ;

	# Don't bother with ps, since nginx changes its process names and
	#   "ps -eo args" doesn't work as expected.
	# WAS: See if nginx process is running.
	#my $ps= (`ps -eo args|grep '[n]ginx'`)[0] ;
	#if ($ps) {
	#    ($nginx, $nginx_args)= $ps=~ /^(\S+)(.*)/ ;
	#    # These are two command-line arguments to the nginx executable.
	#    ($server_root)= $nginx_args=~ /[ \t]-p[ \t]*(\S+)/ ;
	#    ($server_conf_file)= $nginx_args=~ /[ \t]-c[ \t]*(\S+)/ ;
	#}

	# Otherwise, search the path for an appropriate executable.
	chomp($nginx= `which nginx`) ;

	return  unless $nginx ;
	#die "ERROR: Can't find the nginx executable file.\n"  unless $nginx ;

	my $nginx_V= `'$nginx' -V 2>&1` ;
	($server_root)= $nginx_V=~ /\s--prefix=(\S+)/  unless $server_root ;
	$server_root//= '/usr/local/nginx' ;       # as per nginx docs
	($server_conf_file)= $nginx_V=~ /\s--conf-path=(\S+)/  unless $server_conf_file ;
	$server_conf_file//= File::Spec->catdir($server_root, 'conf', 'nginx.conf') ;   # as per nginx docs
	return (undef, $server_root)  unless $server_conf_file ne '' ;
	#die "ERROR: Can't find nginx config file name.\n"  unless $server_conf_file ne '' ;

	# nginx documentation says that "include" directives are resolved
	#   relative to the --prefix directory, but this is not true--
	#   they are resolved relative to the directory containing nginx.conf .
	# See http://mailman.nginx.org/pipermail/nginx/2016-April/050456.html
	my($server_conf_root)= $server_conf_file=~ m#^(.*)/[^/]+$# ;
	$server_conf_file= File::Spec->catdir($server_conf_root, $server_conf_file)
	    unless $server_conf_file=~ m#^/# ;
	...
	if ((stat(_))[4] or ($mode & 022)) {
	    ...
	    $server_conf_full= read_and_expand_nginx_conf($server_conf_file, $server_conf_root) ;
	}


    } elsif ($server_type eq 'lighttpd') {
	# See if lighttpd process is running.
	my $ps= (`ps -eo args|grep '[l]ighttpd'`)[0] ;

	# The -f option is the config file used.
	($server_conf_file)= $ps=~ /[ \t]-f[ \t]*(\S+)/  if $ps ;
	$server_conf_file//= '/etc/lighttpd/lighttpd.conf' ;
	...
	if ((stat(_))[4] or ($mode & 022)) {
	    ...
	    $server_conf_full= read_and_expand_lighttpd_conf($server_conf_file) ;
	}
    }

    return ($server_conf_full, $server_root) ;
}


# Simple routine to print message, since it's used for each server type
#   in get_server_config_unix() .
sub unsafe_conf_file_warning {
    my($server_conf_file)= @_ ;

    print <<EOM ;

***************************************************************************
*** This installation wizard tries to read certain values from the Web
*** server's configuration files.  For security, we won't use a main server
*** configuration file that is not owned by the root user, or one that is
*** writable by anyone but the root user.  The file $server_conf_file
*** doesn't look safe in these ways, so we're not using it.  Instead, we'll
...
# Get $RUN_AS_USER and $RUN_AS_GROUP as needed, by either reading from server
#   config or asking the user.
# This isn't needed for Windows, and shouldn't be called when installing on it.
sub get_run_as_user_group {
    my($run_method, $server_type, $server_conf_full)= @_ ;
    my($run_as_user, $run_as_group) ;

    if ($run_method eq 'c' or $run_method eq 'm') {

	if ($server_type eq 'Apache') {
	    # Find User and Group directives.
	    $server_conf_full=~ /^\s*User\s+['"]?(?:([\w-]+)|#(-?\d+))/mi  and $run_as_user= $+ ;
	    $server_conf_full=~ /^\s*Group\s+['"]?(?:([\w-]+)|#(-?\d+))/mi and $run_as_group= $+ ;

	} elsif ($server_type eq 'lighttpd') {
	    # Find server.username and server.groupname options.
	    $server_conf_full=~ /^\s*server\.username\s*=\s*['"]([\w-]+)/mi  and $run_as_user= $1 ;
	    $server_conf_full=~ /^\s*server\.groupname\s*=\s*['"]([\w-]+)/mi and $run_as_group= $1 ;
	}


	# If we couldn't determine $RUN_AS_USER (including with "other" server
	#   type), ask the user.
	if (!defined($run_as_user)) {
	    $run_as_user= ask_user(<<EOQ,
Every program on Unix (including the Web server) runs as a certain user,
which can be identified with either a name or a number.  Both the username
and numeric user ID can be found in the file /etc/passwd .  Your webmaster
will know the answer to this.
EOQ
		'What is the username or numeric user ID that the Web server runs as?',

		sub {
		    my($u)= @_ ;
		    no warnings 'numeric' ;
		    my $uid= $u==0  ? getpwnam($u)  : $u ;
		    print "There isn't a user named \"$u\" on this machine.\n"
			unless $uid>0 or $u eq 'root' ;
		    $uid>0 or $u eq 'root' ;
		}) ;
	}


	# If we couldn't determine $RUN_AS_GROUP (including with "other" server
	#   type), ask the user.
	if (!defined($run_as_group)) {
	    my $primary_group_id= $run_as_user=~ /^\d+$/
		?  (getpwuid($run_as_user))[3]
		:  (getpwnam($run_as_user))[3] ;
	    my $primary_group_name= getgrgid($primary_group_id) ;
	    $run_as_group= ask_user(<<EOQ,
Similar to a program's user, a program also runs as a "group".  The default
group for the user "$run_as_user" is "$primary_group_name",
If the Web server runs as a different group, enter it here.  Your webmaster
will know the answer to this.
EOQ
		'Which group does the Web server run as?',
		sub {
		    my($g)= @_ ;
		    no warnings 'numeric' ;
		    my $gid= $g==0  ? getgrnam($g)  : $g ;
		    print "There isn't a group named \"$g\" on this machine.\n"
			unless $gid>0 or $g eq 'root' ;
		    $gid>0 or $g eq 'root' ;
		},
		$primary_group_name) ;
	}
    ...
    } elsif ($run_method eq 'e') {

	my $yn= ask_user(<<EOQ, 'Do you plan to start CGIProxy as the root user? [y/n]', qr/^[yn]/i) ;
On Unix (including Linux), you can't make a Web server use the normal "port"
unless you're running the server as the "root user".  This is fine if you
don't mind having a port number in your URL, like "https://example.com:12345"
(which uses port number 12435).  However, if you want to use a URL without
a port number (which is required to get past some censors), you need to start
CGIProxy as root.  For safety, if you do this, you should set \$RUN_AS_USER
and \$RUN_AS_GROUP too (we'll ask you about those next).
EOQ
	if ($yn=~ /^y/i) {
	    $run_as_user= ask_user(<<EOQ,
It's usually not safe to run the embedded server as the "root" user, so please
enter an existing username or numeric user ID that the embedded server should
change to after setting up the right port.  If you really want to run this as
the root user, enter "root" here (not recommended).
EOQ
		'Which user should the embedded server run as?',
		sub {
		    my($u)= @_ ;
		    no warnings 'numeric' ;
		    my $uid= $u==0  ? getpwnam($u)  : $u ;
	    ...
	    $run_as_group= ask_user(<<EOQ,
Similar to a program's user, a program also runs as a "group".  The default
group for the user you entered is "$primary_group_name".  You can use this
group by entering nothing, or you can enter another existing group here.
EOQ
		'Which group should the embedded server run as?',
		sub {
		    my($g)= @_ ;
		    no warnings 'numeric' ;
		    my $gid= $g==0  ? getgrnam($g)  : $g ;
	    ...
	    # We need something for $RUN_AS_USER and $RUN_AS_GROUP only for when
	    #   directories are created, so try current user and group, or else
	    #   script's owner and group if current user is root.
	    ($run_as_user, $run_as_group)= $>!=0  ? ($>, (split(' ', $)))[0])
						  : ((stat($ZERO))[4], (stat(_))[5]) ;
	}



    } else {   # $run_method eq 'f'
	# We need something for $RUN_AS_USER and $RUN_AS_GROUP only for when
	#   directories are created, so try current user and group, or else
	#   script's owner and group if current user is root.
	($run_as_user, $run_as_group)= $>!=0  ? ($>, (split(' ', $)))[0])
...
# They default to owner and group of this script file.
# Better to set $RUN_AS_USER and $RUN_AS_GROUP explicitly.
# This is called both from init() and from 'install'.
# Don't do this on Windows.
sub fix_run_as_user_group {
    if ($^O ne 'MSWin32') {
	no warnings 'numeric' ;
	if ($RUN_AS_USER eq '') {
	    $RUN_AS_USER_ID= (stat($ZERO))[4] ;
	    $RUN_AS_USER= getpwuid($RUN_AS_USER_ID) ;
	} elsif ($RUN_AS_USER=~ /[\D]/) {
	    $RUN_AS_USER_ID= getpwnam($RUN_AS_USER) ;
	} else {   # $RUN_AS_USER is numeric
	    ($RUN_AS_USER, $RUN_AS_USER_ID)= ((scalar getpwuid($RUN_AS_USER)), $RUN_AS_USER) ;
	}
	if ($RUN_AS_GROUP eq '') {
	    $RUN_AS_GROUP_ID= (stat($ZERO))[5] ;
	    $RUN_AS_GROUP= getgrgid($RUN_AS_GROUP_ID) ;
	} elsif ($RUN_AS_GROUP=~ /[\D]/) {
	    $RUN_AS_GROUP_ID= getgrnam($RUN_AS_GROUP) ;
	} else {   # $RUN_AS_GROUP is numeric
	    ($RUN_AS_GROUP, $RUN_AS_GROUP_ID)= ((scalar getgrgid($RUN_AS_GROUP)), $RUN_AS_GROUP) ;
	}
    }
}


# Ensure $LOCAL_LIB_DIR is an absolute path-- if needed, resolve it relative
#   to the script owner's home directory, or on Windows to the current user's
#   home directory.
# jsm-- the way $LOCAL_LIB_DIR works is messy and should be fixed.  :/
sub fix_local_lib_dir {
    die "\$LOCAL_LIB_DIR must not contain \"..\" .\n"  if $LOCAL_LIB_DIR=~ /\.\./ ;
    if ($^O eq 'MSWin32') {
	if (!($LOCAL_LIB_DIR=~ m#^(?:[A-Za-z]:)?(?:/|\\)#)) {
	    my $home_dir= "$ENV{HOMEDRIVE}$ENV{HOMEPATH}" ;
	    die "Sorry, can't figure out home directory.\n"
		unless $home_dir=~ m#^(?:[A-Za-z]:)?(?:/|\\)# ;
	    $LOCAL_LIB_DIR= File::Spec->catdir($home_dir, $LOCAL_LIB_DIR) ;
	}
    } else {
	if ($LOCAL_LIB_DIR=~ m#^[^/]#) {
	    my $home_dir= (getpwuid( (stat($ZERO))[4] ))[7] ;   # of script file owner
	    $LOCAL_LIB_DIR= File::Spec->catdir($home_dir, $LOCAL_LIB_DIR) ;
	}
    }
}
...
# Allow user to modify all config variables via a menu.
sub config_menu {
...
To configure CGIProxy, select what to configure below.  For short descriptions
of all variables, see https://jmarshall.com/tools/cgiproxy/options.html .
For complete descriptions, see the comments in the configuration section near
the top of $ZERO .
EOI

    my $catlist= join('', map { ($_+1) . ". $CONFIG_CATEGORIES[$_]\n" } 0..$#CONFIG_CATEGORIES) ;
    while (my $cat_to_edit= ask_user($catlist,
				     'What do you want to configure (0 to save and exit)? [0-8]',
				     qr/^[0-8]$/ ) )
    {
	$cat_to_edit= $CONFIG_CATEGORIES[$cat_to_edit-1] ;   # convert from number to name
	my $var_to_edit ;
	do {
	    my $varname ;
	    my $varlist= join('', map { scalar($varname= $CONFIG_CATEGORIES{$cat_to_edit}[$_],
						 ($_+1) . ". $varname"
						 . ($varname=~ /^[\@\%]/  ? ''  : ' = ' . dump_var_by_name($varname))
						 . "\n") }
				      0..$#{$CONFIG_CATEGORIES{$cat_to_edit}} ) ;
	    $var_to_edit= ask_user($varlist,
				   'Which variable do you want to set (0 to exit) ? [0-'
				     . @{$CONFIG_CATEGORIES{$cat_to_edit}} . ']',
				   sub {
				       return ($_[0]=~ /^\d+$/
					       and $_[0]<=@{$CONFIG_CATEGORIES{$cat_to_edit}} ) ;
				   } ) ;
	    if ($var_to_edit) {   # can't use last from do{} block  :P
		$var_to_edit= $CONFIG_CATEGORIES{$cat_to_edit}[$var_to_edit-1] ;
		my($sigil, $vname)= (substr($var_to_edit, 0, 1), substr($var_to_edit, 1)) ;
		no strict 'refs' ;

		if ($sigil eq '$') {
		    # jsm-- could set up validation for each variable, with a hash of subroutines
		    my $newval= ask_user("$var_to_edit:\n$OPTION_DESCRIPTIONS{$var_to_edit}\n\n",
					 "What should $var_to_edit be set to?",
					 undef,
					 ${substr($var_to_edit, 1)} ) ;
		    $newval= undef  if $newval eq '' ;    # a bit hacky... no way to set to ''
		    ${substr($var_to_edit, 1)}= $newval ;    # remove sigil


		} elsif ($sigil eq '@') {
		    my $item ;
		    do {
			my $itemlist= join('', map { ($_+1) . ". $$vname[$_]\n" } 0..$#$vname )
				   || "<< $var_to_edit is currently empty. >>\n" ;
			$item= ask_user("$var_to_edit:\n$OPTION_DESCRIPTIONS{$var_to_edit}\n\n"
					. "Here are the current items in the list $var_to_edit:\n\n$itemlist\n",
					"Enter a number to delete that item from $var_to_edit,\nor \"a\" to add an item, or 0 to exit:",
					sub {
					    return 1 if $_[0]=~ /^[Aa]/ ;
					    return ($_[0]=~ /^\d+$/
						    and $_[0]<=@$vname ) ;
			...
			if ($item=~ /^[Aa]/) {
			    push(@$vname, ask_user('', "Enter a new item to add to $var_to_edit:")) ;
			} elsif ($item) {
			    splice(@$vname, $item-1, 1) ;
			}
		    } while $item ;
...
$var_to_edit is a \"hash\", which is a set of key-value pairs.  In the
list below, the key is the part before \"=>\", and the value is the part
after it.  In a hash, there cannot be more than one item with the same key.
EOE
		    my $item ;
		    do {
			my @keylist= sort keys %$vname ;
			my $itemlist= join('', map { $_ . ". $keylist[$_-1]  =>  $$vname{$keylist[$_-1]}\n" }
						   1..@keylist )
				   || "<< $var_to_edit is currently empty. >>\n" ;
			$item= ask_user("$var_to_edit:\n$OPTION_DESCRIPTIONS{$var_to_edit}\n\n"
					. "Here are the current items in the hash $var_to_edit:\n\n$itemlist\n",
					"Enter a number to delete that item from $var_to_edit,\nor \"a\" to add or change an item, or 0 to exit:",
					sub {
					    return 1 if $_[0]=~ /^[Aa]/ ;
					    return ($_[0]=~ /^\d+$/
						    and $_[0]<=@keylist ) ;
			...
			if ($item=~ /^[Aa]/) {
			    my $key= ask_user('', "Enter the key of the new or existing item, or nothing to exit:") ;
			    my $value= (exists $$vname{$key})
				? ask_user('', "Change the value of \$$vname\{'$key'\} to what?", undef, $$vname{$key})
				: ask_user('', "Set a new item \$$vname\{'$key'\} to what?") ;
			    $$vname{$key}= $value ;
			} elsif ($item) {
			    delete $$vname{$keylist[$item-1]} ;
			}
		    } while $item ;
		}
	    }
...
# Prompt a user, get a response, and confirm that it matches $allowed.
# $question is the initial text, and $prompt is the part that is repeated
#   until the response matches $allowed .
# $allowed must be either a regular expression created with qr// , or a
#   reference to a subroutine that returns true or false based on whether its
#   first argument is an allowed value.
sub ask_user {
    my($question, $prompt, $allowed, $default)= @_ ;
    my($is_allowed, $response) ;
    local $/= "\n" ;

    if (ref($allowed) eq 'Regexp') {
	$is_allowed= sub { $_[0]=~ $allowed } ;
    } elsif (ref($allowed) eq 'CODE') {
	$is_allowed= $allowed ;
    }

    # On Windows, this will show an error when trying to instantiate a second
    #   Term::Readline object, so just store one in a global.
    $TERM_RL||= Term::ReadLine->new('') ;
    local $Term::ReadLine::termcap_nowarn= 1 ;   # undocumented, but suppresses a warning on Windows
    $TERM_RL->ornaments(0) ;    # remove underlines in prompts

    # Some systems don't have a good Term::ReadLine package installed, so
    #   accommodate those.  In particular, the Term::ReadLine::Stub package
    #   doesn't support the preput (default) parameter in readline().  :(
    print "\n", $question, "\n" ;
    if (Term::ReadLine->ReadLine()!~/Stub$/) {
	do {
	    $response= $TERM_RL->readline("---> $prompt ", $default) ;
	} until !$is_allowed or &$is_allowed($response) ;

    } else {
	do {
	    my $defaultst ;
	    if ($default ne '') {
		print "---> Enter nothing to leave value unchanged, or a single space to clear.\n" ;
		$defaultst= "[\"$default\"] " ;
	    }
	    $response= $TERM_RL->readline("---> $prompt $defaultst") ;
	    $response= $response eq ''   ? $default
		     : $response eq ' '  ? undef
		     :                     $response ; 
	} until !$is_allowed or &$is_allowed($response) ;
    }
...
# Perform an HTTP HEAD request to localhost on the given port (default=443),
#   and return the "Server:" header value.
# Assume SSL unless the port is 80.
# Returns undef on error or lack of Server: header.
sub get_server_header {
    ...
    eval {
	# Connect via port 443 and SSL if available, else via port 80.
	if ($port==80) {
	    return undef unless newsocketto_simple('S_UID', 'localhost', 80) ;
	} else {
	    eval { require Net::SSLeay } ;
	    die "Can't load Net::SSLeay module: $@\n"  if $@ ;
	    $Net::SSLeay::ssl_version= 12 ;   # start with TLS 1.2, then try lower as needed
	    return undef unless newsocketto_simple('S_PLAIN', 'localhost', $port) ;
	    my $ssl_obj= tie(*S_UID, 'SSL_Handle', \*S_PLAIN) ;
	    Net::SSLeay::connect($ssl_obj->{SSL})  or die "Can't SSL connect: $!\n" ;
	}

	# Use HTTP/1.0 here for simplicity, but still include Host: header.
	select((select(S_UID), $|= 1)[0]) ;   # autoflush on S_UID
	print S_UID "HEAD / HTTP/1.0\nHost: localhost\n\n" ;
	local($/)= undef ;
	$resp= <S_UID> ;   # blocks until connection closed....
    } ;
    close(S_UID) ;
    untie(*S_UID), close(S_PLAIN) unless $port==80 ;

    return undef unless $resp ne '' ;

    my($headers, $body)= split(/\015?\012\015?\012/, $resp) ;
    my($server_header)= $headers=~ /^Server:[ \t]*([^\015\012]+)/mi ;
    return $server_header ;
}



# Try to discern the UID and GID the HTTP server runs as.
# You'd think it would be easy.  :/
sub get_httpd_uid_gid_via_script {
    my($ret_uid, $ret_gid) ;
    local(*S_PLAIN, *S_UID) ;

    # getlogin gets login name even when su'ing .
    (my $logname= getlogin) ne '' or die "ERROR: can't get login name.\n" ;
    my($uid, $gid, $home_dir)= (getpwnam($logname))[2, 3, 7] ;
    $PROXY_DIR= File::Spec->catdir($home_dir, $PROXY_DIR)  unless $PROXY_DIR=~ m#^/# ;


    # First, try to learn the UID through calling a tiny CGI script.
    my($uid_cgi, $uid_cgi_full) ;
    do {
	$uid_cgi= random_string(20) . '.cgi' ;
	$uid_cgi_full= File::Spec->catdir($home_dir, ($^O eq 'darwin' ? 'Sites' : 'public_html'), $uid_cgi) ;
    } while -e $uid_cgi_full ;   # Avoid clobbering another file.
    open(my $uid_cgi_fh, '>', $uid_cgi_full)  or die "can't open $uid_cgi_full : $!\n" ;
    print $uid_cgi_fh <<'EOF' ;
#!/usr/bin/perl
print "Content-Type: text/plain\n\n$>;$)" ;
EOF
    close($uid_cgi_fh) ;
    if ($>==0) {
	chown($uid, $gid, $uid_cgi_full)  or die "can't chown $uid_cgi_full: $!\n" ;
    }
    chmod(0755, $uid_cgi_full)  or die "can't chmod 0755 $uid_cgi_full: $!\n" ;

    # GET the script output via HTTP.
    # Connect via port 443 and SSL if available, else via port 80.
    my($success) ;
    eval { require Net::SSLeay } ;
    $Net::SSLeay::ssl_version= 12  unless $@ ;   # start with TLS 1.2, then try lower as needed
    if (!$@ and newsocketto_simple('S_PLAIN', 'localhost', 443)) {
	my $ssl_obj= tie(*S_UID, 'SSL_Handle', \*S_PLAIN) ;
	# Support the TLS SNI extension.  Thanks to Carsten Gaebler for the patch.
	Net::SSLeay::set_tlsext_host_name($ssl_obj->{SSL}, 'localhost')
	    if defined &Net::SSLeay::set_tlsext_host_name ;
	Net::SSLeay::connect($ssl_obj->{SSL})  or die "Can't SSL connect: $!\n" ;
	$success= 1 ;
    } elsif (newsocketto_simple('S_UID', 'localhost', 80)) {
	$success= 1 ;
    }

    # If we successfully connect to localhost, GET the script output.
    # Use HTTP/1.0 here for simplicity, but still include Host: header.
    if ($success) {
	select((select(S_UID), $|= 1)[0]) ;   # autoflush on S_UID
	print S_UID "GET /~$logname/$uid_cgi HTTP/1.0\nHost: localhost\n\n" ;
	local($/)= undef ;
	my $resp= <S_UID> ;   # blocks until connection closed....
	close(S_UID) ;
	untie(*S_UID) ;
	close(S_PLAIN) ;
	unlink($uid_cgi_full)  or die "can't unlink $uid_cgi_full : $!\n" ;

	my($headers, $body)= split(/\r?\n\r?\n/, $resp) ;
	($ret_uid, $ret_gid)= $body=~ /^(\d+);(\d+)/ ;  # ignore groups after first
	die "Didn't find UID and GID in test response body \"$body\"; headers=[$headers]."
	    unless defined($ret_uid) and defined($ret_gid) ;
    }

    return ($ret_uid, $ret_gid) ;
}



# As well as possible, get environment variables from envvars .
sub get_apache_env {
    # envvars doesn't seem to exist on Windows.
    return undef if $^O eq 'MSWin32' ;

    my $envvars= get_apache_env_path() or return undef ;

    # "Source" envvars and load the resulting environment into a hash.
    my $VAR1 ;
    return undef if $envvars=~ /['"]/ ;
    eval qx{sh -c '. "$envvars" && perl -MData::Dumper -e "print Dumper(\\\\%ENV)"'} ;
    return $VAR1 ;
}

# Try to locate the envvars file, which is normally sourced by apache2ctl to
#   set environment variables that are then used in httpd.conf .
sub get_apache_env_path {
    my $ret ;

    # Try APACHE_ENVVARS environment variable.
    return $ENV{APACHE_ENVVARS}  if -f $ENV{APACHE_ENVVARS} ;

    # Look in common locations of envvars on various operating systems.
    # Better would be to use the right path for each OS....
    -f && return $_  foreach qw(/etc/apache2/envvars /usr/sbin/envvars /etc/sysconfig/httpd) ;

    # Finally, look in PATH for it.
    foreach (split(/:/, $ENV{PATH})) {
	my $envvars_path= File::Spec->catdir($_, 'envvars') ;
	return $envvars_path  if -f $envvars_path ;
    }
...
# Read in entire httpd.conf file, with Include and IncludeOptional directives
#   recursively expanded in place.  Environment variables of the form ${var}
#   are all expanded.  All parameters to all directives will be double-quoted
#   in output, which actually makes them easier to handle later.
# jsm-- must test ServerRoot support....
sub read_and_expand_httpd_conf {
    my($fnames, $apache_env, $server_root, $is_optional)= @_ ;

    $fnames= File::Spec->catdir($server_root, $fnames)  unless $fnames=~ m#^/# ;
    if (-d $fnames) {
	$fnames.= '/'  unless $fnames=~ m#/$# ;
	$fnames.= '*' ;
    }
    my @fnames= glob($fnames) ;
    die "No such file(s) as $fnames , included in httpd.conf .\n"  if !$is_optional and !@fnames ;

    my $ret= [] ;
    foreach my $fname (@fnames) {
	die "ERROR: Can't read $fname , included in httpd.conf .\n"  unless -r $fname ;
	next if $fname=~ /['\\]/ ;
	foreach my $line (`cat '$fname'`) {
	    next if $line=~ /^\s*(?:#|$)/ ;
	    my($directive, $params)= split(' ', $line, 2) ;
	    $directive= lc($directive) ;
	    my @param ;
	    push (@param, $1) while $params=~ /\G("(?:[^"]|\\")*"|'(?:[^']|\\')*'|[^'">\s][^\s>]*)\s*/gc ;
	    my($rest)= $params=~ /\G(.*)/ ;
	    s/^['"]|['"]$//g  foreach @param ;
	    s/\$\{(\w+)\}/$apache_env->{$1}/g  foreach @param ;   # expand environment variables
	    if ($directive eq 'define') {
		$apache_env->{$param[0]}= $param[1] ;
	    } elsif ($directive eq 'undefine') {
		delete $apache_env->{$param[0]} ;
	    } elsif ($directive eq 'serverroot') {
		$server_root= $param[0] ;
	    } elsif ($directive eq 'include') {
		push(@$ret, read_and_expand_httpd_conf($param[0], $apache_env, $server_root, 0)) ;
	    } elsif ($directive eq 'includeoptional') {
		push(@$ret, read_and_expand_httpd_conf($param[0], $apache_env, $server_root, 1)) ;
	    }
	    push(@$ret, join(' ', $directive, (map { "\"$_\"" } @param)), $rest, "\n" )
		unless $directive eq 'include' or $directive eq 'includeoptional' ;
	}
    }
...
# Read in entire nginx.conf file, with include directives recursively expanded
#   in place.
# jsm-- should expand nginx config variables?
sub read_and_expand_nginx_conf {
    my($fnames, $server_conf_root)= @_ ;

    $fnames= File::Spec->catdir($server_conf_root, $fnames)  unless $fnames=~ m#^/# ;
    my @fnames= glob($fnames) ;
    my $ret= [] ;
    foreach my $fname (@fnames) {
	die "ERROR: Can't read $fname , included in nginx.conf .\n"  unless -r $fname ;
	next if $fname=~ /['\\]/ ;
	push(@$ret, /^\s*include\s+(?:"([^"]+)"|'([^']+)'|([^\s;]+))/i
				   ? read_and_expand_nginx_conf($+, $server_conf_root)
		  : /^\s*(?:#|$)/  ? ()
		  : $_ )
	    foreach `cat '$fname'` ;
    }
...
# Read in entire lighttpd.conf file, with include directives
#   recursively expanded in place.
# jsm-- must test....
sub read_and_expand_lighttpd_conf {
    my($fname, $server_root)= @_ ;

    $server_root||= ($fname=~ m#^(.*)/[^/]+$#s)[0] ;
    $fname= File::Spec->catdir($server_root, $fname)  unless $fname=~ m#^/# ;
    die "ERROR: Can't read $fname , included in lighttpd.conf .\n"  unless -r $fname ;

    my $ret= [] ;
    return if $fname=~ /['\\]/ ;
    push(@$ret, /^\s*include\s+["']([^\015\012"']*)/i  ? read_and_expand_lighttpd_conf($1, $server_root)
	      : /^\s*(?:#|$)/  ? ()
	      : $_ )
	foreach `cat '$fname'` ;
...
# Simplified version of newsocketto() .  No support for @BANNED_NETWORKS,
#   proxy, or SOCKS.
sub newsocketto_simple {
    my($S, $host, $port)= @_ ;
    my($success) ;

    # Support IPv6 if available.
    if ($Socket::VERSION>=1.94) {
	# Just get first address returned.
	my($err, $addr)= getaddrinfo($host, $port, { socktype => SOCK_STREAM }) ;
	die "Can't get address for $host : $err"  if $err ;
	no strict 'refs' ;
	$success= socket($S, $addr->{family}, SOCK_STREAM, (getprotobyname('tcp'))[2])
	      && connect($S, $addr->{addr}) ;
	...
	if ($addr) {
	    my $remotehost= pack_sockaddr_in($port, $addr) ;
	    no strict 'refs' ;
	    $success= socket($S, AF_INET, SOCK_STREAM, (getprotobyname('tcp'))[2])
		  && connect($S, $remotehost) ;
	}
    }
    return $success ;
...
# Copy script to one of these, depending on $run_method:  DocumentRoot/$SECRET_PATH/ ,
#   ~/UserDir/$SECRET_PATH/ , or $PROXY_DIR/bin/ .
# For CGI and mod_perl installations, this confirms any final location with the
#   user before copying.
# Returns destination file name.
sub install_script {
    my($run_method, $server_type, $server_conf_full, $server_root,
       $script_mode, $script_dir_mode)= @_ ;
    my($install_dir, $url_guess) ;

    my($script_name)= $ZERO=~ ($^O eq 'MSWin32'  ? qr#([^/\\]+)$#  : qr#([^/]+)$#) ;

    # For FastCGI or embedded server installations, just copy into $PROXY_DIR/bin/ .
    if ($run_method eq 'f' or $run_method eq 'e') {
	$url_guess= "https://localhost/$SECRET_PATH/" ;
	$install_dir= File::Spec->catfile($PROXY_DIR, 'bin') ;
	-d $install_dir or mkdir($install_dir, $script_dir_mode) or die "mkdir [$install_dir]: $!" ;


    # For CGI or mod_perl installations, calculate $install_dir .
    } else {

	# For Windows servers, just ask the user where to install it, since we
	#   never read the server config.
	if ($^O ne 'MSWin32') {

	    # If root user, install under document root.
	    if ($>==0) {
		$url_guess= "https://localhost/$SECRET_PATH/$script_name" ;
		if ($server_type eq 'Apache') {
		    ($install_dir)= $server_conf_full=~ /^\s*DocumentRoot\s+['"]?([^'"\r\n]+)/mi ;
		    $install_dir//= '/usr/local/apache/htdocs' ;   # as per Apache docs
		    if ($install_dir!~ m#^/#) {
			$install_dir= (-d $server_root and $server_root=~ m#^/#)
			    ? File::Spec->catfile($server_root, $install_dir)
			    : undef ;
		    }
		} elsif ($server_type eq 'lighttpd') {
		    ($install_dir)= $server_conf_full=~ m#^\s*server\.document-root\s*=\s*['"]?(/[^'"\r\n]+)#mi ;
		}

	    # Otherwise, install under UserDir or its equivalent.
	    } else {
		my $username= getpwuid($>) ;
		$url_guess= "https://localhost/~$username/$SECRET_PATH/$script_name" ;
		my($user_dir) ;
		if ($server_type eq 'Apache') {
		    # UserDir is somewhat complex-- it can be multiple values, it can be
		    #   a URL or an absolute or relative path, and it can have a wildcard.
		    #   Here we ignore the possibility of multiple values or URLs, and
		    #   confirm with the user.
		    ($user_dir)= $server_conf_full=~ /^\s*UserDir\s+['"]?([^'"\r\n]+)/mi ;
		    $user_dir//= $^O eq 'darwin'  ? 'Sites'  : 'public_html' ;
		    if ($user_dir=~ m#^[\w+.-]+://#) {
			$install_dir= undef ;
		    } elsif ($user_dir!~ m#^/#) {
			$install_dir= File::Spec->catfile((getpwuid($>))[7], $user_dir) ;
		    } elsif ($user_dir=~ /\*/) {
			$user_dir=~ s/\*/$username/ ;
			$install_dir= $user_dir ;
		    } else {
			$install_dir= File::Spec->catfile($user_dir, scalar getpwuid($>)) ;
		    }

		} elsif ($server_type eq 'lighttpd') {
		    ($user_dir)= $server_conf_full=~ /^\s*userdir\.path\s*=\s*['"]?([^'"\r\n]+)/mi ;
		    my($userdir_basepath)= $server_conf_full=~ /^\s*userdir\.basepath\s*=\s*['"]?([^'"\r\n]+)/mi ;
		    if (!defined $user_dir) {
			$install_dir= undef ;
		    } elsif ($user_dir=~ m#^[^/]#) {
			$install_dir= defined $userdir_basepath
			    ? File::Spec->catfile($userdir_basepath, scalar getpwuid($>), $user_dir)
			    : File::Spec->catfile((getpwuid($>))[7], $user_dir) ;
		    } else {
			$install_dir= $user_dir ;
		    }
		}
	...
	# We may have just come from installing modules, so print a visual separator.
	print '=' x 72, "\n" ;

	# Ask or confirm $install_dir with user.
	my $new_install_dir= ask_user( (defined($install_dir)  ? <<EOQD  : <<EOQU ) . <<EON ,
We think you want to install CGIProxy under this directory:
  $install_dir

If you want to install CGIProxy here, just hit <Enter> below.
EOQD
Hmm, we couldn't figure out the right directory to copy CGIProxy into.
You'll need to tell us where to put it.
EOQU

Note that we'll add a subdirectory under that with the name of your
\$SECRET_PATH setting, currently set to "$SECRET_PATH".
EON
				'Which directory should we install CGIProxy under?',
				sub {
				    my($dr)= @_ ;
				    print "That directory doesn't exist.  Please enter an existing directory.\n"
					unless -d $dr ;
				    -d $dr ;
				},
				$install_dir) ;

	# If the user entered a different installation directory, try to figure
	#   out $url_guess.
	if ($new_install_dir ne $install_dir) {
	    # If they just appended to $install_dir, we can handle that.  But nothing else.
	    my $extra ;
	    if (($extra)= $new_install_dir=~ m#\Q$install_dir/\E(.*)#) {
		$url_guess=~ s#\Q$SECRET_PATH\E/#$extra/$SECRET_PATH/# ;
	    } else {
		$url_guess= undef ;
	    }
	}
	...
	# Append $SECRET_PATH subdirectory to the installation directory.
	die "\$SECRET_PATH can't contain \"..\".\n"  if $SECRET_PATH=~ /\.\./ ;
	$install_dir= File::Spec->catfile($install_dir, $SECRET_PATH) if $SECRET_PATH ne '' ;

	# Create $install_dir directory and set its permissions and ownership correctly.
	-d $install_dir or mkdir($install_dir, $script_dir_mode) or die "mkdir [$install_dir]: $!" ;
	if ($^O ne 'MSWin32' and $>==0) {
	    chown($RUN_AS_USER_ID, $RUN_AS_GROUP_ID, $install_dir)
		or die "can't chown $RUN_AS_USER:$RUN_AS_GROUP $install_dir : $!\n" ;
	}
    }


    # Copy script into place, preserving filename when copying.

    my $destfile= File::Spec->catfile($install_dir, $script_name) ;

    # Using cp() instead of copy() will keep permissions, but not owner and group.
    require File::Copy ;
    File::Copy::cp($ZERO, $destfile) or die "Can't copy $ZERO to $destfile : $!\n" ;

    # Modify this script file as needed.
    modify_script_as_needed($destfile) ;

    # Set file ownership and permissions appropriately.
    if ($^O ne 'MSWin32') {
	if ($>==0) {
	    chown($RUN_AS_USER_ID, $RUN_AS_GROUP_ID, $destfile)
		or die "can't chown $RUN_AS_USER:$RUN_AS_GROUP $destfile $!" ;
	} elsif ($)=~ /\b$RUN_AS_GROUP_ID\b/) {
	    chown(-1, $RUN_AS_GROUP_ID, $destfile)
		or die "can't chown :$RUN_AS_GROUP $destfile $!" ;
	}
    }
    chmod($script_mode, $destfile) or die "Can't chmod $destfile : $!\n" ;    # in case it wasn't set before

    print "\nInstalled script to \"$destfile\".\n\n" ;

    return ($destfile, $url_guess) ;
}



# Modify the named instance of nph-proxy.cgi .  This includes:
#   - replace first line with "#!perl" if on Windows
#   - replace $PROXY_DIR setting with absolute pathname.
# Hacky.
sub modify_script_as_needed {
    my($filename)= @_ ;
    my($fh, $contents) ;
    ...
    $contents=~ s/^#!.*/#!perl/  if $^O eq 'MSWin32' ;
    my $safe_pd= $PROXY_DIR ;
    $safe_pd=~ s/(['\\])/\\$1/g ;
    $contents=~ s/^\s*\$PROXY_DIR\s*=\s*.*/\$PROXY_DIR= '$safe_pd' ;/m ;
...
# Optionally add cron job to periodically purge database.
# Returns 1 if successful, 0 if not, or undef if crontab is not available.
# Of course, this only works on Unix.
sub add_cron_job_purge_db {
    my($installed_script)= @_ ;

    if ($^O eq 'MSWin32') {
	print <<EOM ;

***************************************************************************
You need to add a job to periodically purge CGIProxy's database, using
Windows' Task Scheduler.  Otherwise, your database will accumulate lots of
unnecessary data.  Please add the following command to be run either daily
or hourly:

  $installed_script purge-db
...
EOM
    ...
    # crontab is not available on all systems.
    if (`which crontab 2>/dev/null` eq '') {
	print <<EOM ;

We tried to add a cron job to periodically purge CGIProxy's database, but 
this server doesn't have the "crontab" utility that we need to do this.

You need to add this cron job yourself, possibly with a configuration utility
that your hosting provider gives you.  Otherwise, your database will accumulate
lots of unnecessary data.  For example, to purge your database every morning
at 2:13am, add this cron job:

  13 2 * * * $installed_script purge-db
...
EOM
    ...
    # Set -u parameter to crontab command.
    my $rausafe= $RUN_AS_USER ;
    $rausafe=~ s/'//g ;
    my $cuparam= ($>==0) && ($RUN_AS_USER ne '')  ? "-u '$rausafe'"  : '' ;

    # Check for existing cron job.
    my(@cron_jobs)= grep { !/^\s*#/ } `crontab $cuparam -l 2>/dev/null` ;
    foreach (@cron_jobs) {
	my @f= split(/\s+/, $_, 6) ;
	my $u= ($>==0) && ($RUN_AS_USER ne '')
	    ? "User \"$RUN_AS_USER\" already has"  : 'You already have' ;
	print(<<EOM), return 1
$u a cron job to purge the database, so we
won't add one.
EOM
	    if $f[5]=~ /^\Q$installed_script\E\s+purge-db/ ;
    }


    # pick a random minute between 2:00am and 4:00am
    my $hour= 2+int(rand(2)) ;
    my $minute= int(rand(60)) ;
    my $cron_job= "$minute $hour * * * $installed_script purge-db\n" ;
    push(@cron_jobs, $cron_job) ;

    if (open(CRON, '|-', "crontab $cuparam -")) {
	print CRON @cron_jobs ;
	close(CRON) ;
	my $min2= sprintf("%02d", $minute) ;
	my $cron_user= $>==0  ? $RUN_AS_USER  : getpwuid($>) ;
	print <<EOM ;
Added a cron job for user "$cron_user" to purge the CGIProxy database every
morning at $hour:${min2}am .
...
We tried to add a cron job to periodically purge CGIProxy's database, but we
couldn't for this reason: $!

You need to add this cron job yourself, possibly with a configuration utility
that your hosting provider gives you.  Otherwise, your database will accumulate
lots of unnecessary data.  For example, to purge your database every morning
at 2:13am, add this cron job for the user "$RUN_AS_USER" :
...
#----------------------------------------------------------------------
#   Configuration option short descriptions
#----------------------------------------------------------------------

sub get_option_descriptions {
    return if %OPTION_DESCRIPTIONS ;
    %OPTION_DESCRIPTIONS= () ;

    my $all_descriptions= <<'EOD' ;
...
$DB_USER, $DB_PASS [none]
    CGIProxy accesses the database with this $DB_USER and $DB_PASS.
...
EOD
	...
	my($names, $desc)= split(/\n/, $block, 2) ;
	$names=~ s/\s*\[.*?\]\s*//g ;
	$OPTION_DESCRIPTIONS{$_}= $desc  foreach split(/\s*,\s*/, $names) ;
    }
}
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>الوكيل لا يمكن الوصول إلى ذلك الخادم، آسف </title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>> نأسف  أنت غير مصرح لك الدخول الى الخادم البديل</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>FTP خطا بروتكول ناقل الملفات</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>%s في %s ادخل اسم الحساب وكلمة  المررور ل</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>يحظر استرجاع عناوين المواقع الآمن عبر وكيل غير آمن</title>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>الخادم البديل لا يستطيع اعادة الحلقة من تلقاء نفسه</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title> الصفحة تمت يونيكود تالف</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title> UTF-16 الصفحة التي طلبت على ما يبدو في تنسيق في </title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>الملفات المضغوطه ليست مقبوله، لكن الموقع بعث ملف مضغوط</title></head>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>(non-text files)الوكيل لا يمكن إحالة ملفات غير نصية</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>برامج سكريبت ممنوع أن ينفذ</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>تحذير: تدخل العنوان تظهر فيه هويتك</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Der Proxy kann auf diesen Server nicht zugreifen, Entschuldigung.</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Du darfst diesen Proxy nicht nutzen, Entschuldigung.</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>FTP-Verzeichnis von %s</title>
</head>
<body>
%s
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>FTP-Fehler</title></head>
<body>
<h1>FTP-Fehler</h1>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Gib Benutzername und Passwort für %s bei %s ein</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Aufruf sicherer URLs durch unsicheren Proxy ist verboten.</title>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Der Proxy darf sich nicht selbst nutzen</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Seite enthält fehlgeformtes Unicode</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Seite nutzt UTF-16 Kodierung, was von dieser Perl-Version nicht
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>WARNUNG: SSL nicht unterstützt, Du betrittst den nicht-anonymen Bereich!</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Komprimierter Inhalt nicht unterstützt, wurde aber vom Server geschickt.</title></head>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Proxy kann keine Nicht-Text-Dateien durchleiten</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Script-Inhalte blockiert</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>WARNUNG: Du betrittst den nicht-anonymen Bereich!</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Este proxy no puede accesar al server, perdón.</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Usted no tiene permiso para utilizar este proxy, perdón.</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Error FTP</title></head>
</head>
<body>
<h1>Error FTP</h1>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Ingresar usuario y password para %s en %s</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>La recuperación de las URL segura a través de un proxy no seguro está prohibido.</title>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Proxy no puede regresar a si mismo</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>La página tiene Unicode mal formado</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>La página usa codificación UTF-16, la cual no esta soportada por esta versión
      de Perl</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>CUIDADO: SSL no soportado, entrando a área no-anonima!</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Contenido comprimido no soportado,pero fue enviado por el servidor.</title></head>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Proxy no puede reenviar archivos no de texto</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>Contenido del Script bloqueado</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>CUIDADO: Accesando area no-anonima!</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>این پراکسی نمی‌تواند به سرور مورد نظر دسترسی داشته باشد.</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>با پوزش، شما مجاز به استفاده از این پراکسی نیستید.</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>دایرکتوری FTP %s</title>
</head>
<body>
%s
<h1>FTP سرور در %s</h1>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>FTP خطای</title></head>
<body>
<h1>FTP خطای</h1>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>نام کاربری و رمز عبور خود را برای %s در %s وارد کنید.</title></head>
<body>
<h1>نیازمند اجازه </h1>
%s
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>بازیابی داده‌های یک آدرس اینترنتی امن از طریق یک پراکسی ناامن مجاز نیست.</title>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>پروکسی نمی‌تواند از طریق خودش لوپ بک کند.</title></head>
<body>
...
'<html%s>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1" /><title>صفحه دارای یونیکد ناقص است</title></head>
<body>
